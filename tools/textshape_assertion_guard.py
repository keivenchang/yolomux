...
TEXT_SHAPE_ASSERTION_ALLOWLIST: Final[dict[str, str]] = {
    ...
    "tests/test_common.py:test_package_and_runtime_versions_use_one_source": "The single version module and package metadata are a source-of-truth contract that cannot be established by runtime behavior alone.",
    ...
    "tests/test_dev_restart_script.py:test_boot_restart_refuses_duplicate_without_blind_listener_kill": "The boot script must refuse an unverified duplicate and reserve force replacement for the verified instance lease; that negative launcher wiring is safest to check from source.",
    ...
    "tests/test_filesystem_access_policy.py:test_one_owner_builds_every_filesystem_job_descriptor": "The one-owner scan IS a source-shape contract: a filesystem job descriptor built by hand anywhere in app.py, batchd.py or tests/ is the second construction site that reintroduces the cross-port authorization bypass. Its allowlist is keyed by occurrence count so a second copy of an allowlisted literal still fails, and it was watched firing on a reverted fixture.",
    "tests/test_launcher_row_wiring.py:test_launcher_has_no_inherited_global_primary_port_export": "The retired global scheduler primary-port export, the Darwin launchctl setenv, and the two dropped early/managed instance-port vars must remain ABSENT from the live launcher; a negative source search is the only way to prove a removed global env export never returns, and the launcher starts real dev servers so it cannot be exercised at runtime here.",
...
TEXT_SHAPE_ASSERTION_INVENTORY_SHA256: Final[str] = "ca7b6ccf62f9292e5b193e3948e3aeadd5363484286f4ed06742cf1d06768618"

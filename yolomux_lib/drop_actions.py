# SPDX-FileCopyrightText: Copyright (c) 2026 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0
"""Read-only server-side file drop actions."""

from __future__ import annotations

import csv
import json
import shutil
import statistics
import subprocess
from collections import Counter
from io import StringIO
from pathlib import Path
from typing import Any

from . import filesystem


MAX_ACTION_PATHS = 8
MAX_PREVIEW_LINES = 80
MAX_SCAN_LINES = 2_000
MAX_ERROR_LINES = 80
MAX_DATA_ROWS = 2_000
ERROR_TOKENS = ("error", "exception", "traceback", "failed", "fatal", "warn", "warning")


def _payload_paths(payload: dict[str, Any]) -> list[str]:
    raw_paths = payload.get("paths")
    if not isinstance(raw_paths, list):
        raw_paths = [payload.get("path", "")]
    paths: list[str] = []
    for raw in raw_paths:
        path = str(raw or "").strip()
        if path and path not in paths:
            paths.append(path)
        if len(paths) >= MAX_ACTION_PATHS:
            break
    return paths


def _result(action: str, title: str, body: str, paths: list[str], **extra: Any) -> dict[str, Any]:
    return {"ok": True, "action": action, "title": title, "body": body, "paths": paths, **extra}


def _format_info(path: str) -> str:
    info = filesystem.path_info(path)
    file_path = Path(str(info["path"]))
    size = ""
    mtime = ""
    if file_path.exists() and file_path.is_file():
        stat = file_path.stat()
        size = f"\nsize: {stat.st_size} bytes"
        mtime = f"\nmodified: {stat.st_mtime:.0f}"
    repo = info.get("repo_root") or ""
    relative = info.get("relative_path") or ""
    repo_lines = f"\nrepo: {repo}" if repo else ""
    rel_lines = f"\nrelative: {relative}" if relative else ""
    return f"path: {info['path']}\nkind: {info['kind']}{size}{mtime}{repo_lines}{rel_lines}"


def _head_result(action: str, paths: list[str]) -> dict[str, Any]:
    blocks = []
    for path in paths:
        payload = filesystem.read_file(path)
        lines = str(payload.get("content", "")).splitlines()
        preview = "\n".join(lines[:MAX_PREVIEW_LINES])
        suffix = f"\n... truncated after {MAX_PREVIEW_LINES} lines" if len(lines) > MAX_PREVIEW_LINES else ""
        blocks.append(f"## {path}\n\n{preview}{suffix}")
    return _result(action, "File preview", "\n\n".join(blocks), paths)


def _info_result(action: str, paths: list[str]) -> dict[str, Any]:
    return _result(action, "File information", "\n\n".join(_format_info(path) for path in paths), paths)


def _log_errors_result(action: str, paths: list[str]) -> dict[str, Any]:
    blocks = []
    for path in paths:
        payload = filesystem.read_file(path)
        matches = []
        counts: Counter[str] = Counter()
        for number, line in enumerate(str(payload.get("content", "")).splitlines()[:MAX_SCAN_LINES], start=1):
            lower = line.lower()
            matched = [token for token in ERROR_TOKENS if token in lower]
            if not matched:
                continue
            for token in matched:
                counts[token] += 1
            if len(matches) < MAX_ERROR_LINES:
                matches.append(f"{number}: {line}")
        summary = ", ".join(f"{token}={count}" for token, count in counts.most_common()) or "none"
        body = "\n".join(matches) if matches else "No obvious error or warning lines found."
        blocks.append(f"## {path}\n\nsummary: {summary}\n\n{body}")
    return _result(action, "Log errors and warnings", "\n\n".join(blocks), paths)


def _json_stats(content: str) -> str:
    data = json.loads(content)
    if isinstance(data, list):
        keys = Counter()
        for item in data[:MAX_DATA_ROWS]:
            if isinstance(item, dict):
                keys.update(item.keys())
        common = ", ".join(f"{key}({count})" for key, count in keys.most_common(20))
        return f"type: JSON array\nitems: {len(data)}\nobject keys: {common or 'not object rows'}"
    if isinstance(data, dict):
        keys = ", ".join(map(str, list(data.keys())[:40]))
        return f"type: JSON object\nkeys: {keys}"
    return f"type: {type(data).__name__}\nvalue: {data!r}"[:4_000]


def _csv_stats(content: str, delimiter: str) -> str:
    reader = csv.DictReader(StringIO(content), delimiter=delimiter)
    rows = []
    for row in reader:
        rows.append(row)
        if len(rows) >= MAX_DATA_ROWS:
            break
    fields = list(reader.fieldnames or [])
    lines = [f"rows scanned: {len(rows)}", f"columns: {', '.join(fields) if fields else 'none'}"]
    for field in fields[:20]:
        values = [str(row.get(field, "") or "") for row in rows]
        missing = sum(1 for value in values if not value.strip())
        numeric = []
        for value in values:
            try:
                numeric.append(float(value))
            except ValueError:
                continue
        if numeric:
            avg = statistics.fmean(numeric)
            lines.append(f"{field}: numeric count={len(numeric)} min={min(numeric):.4g} max={max(numeric):.4g} avg={avg:.4g} missing={missing}")
        else:
            distinct = len(set(value for value in values if value.strip()))
            lines.append(f"{field}: text distinct={distinct} missing={missing}")
    chart = _numeric_chart(rows, fields)
    if chart:
        lines.append("\nchart:\n" + chart)
    return "\n".join(lines)


def _numeric_chart(rows: list[dict[str, str]], fields: list[str]) -> str:
    for field in fields:
        values = []
        for row in rows:
            try:
                values.append(float(str(row.get(field, "") or "")))
            except ValueError:
                continue
        if len(values) < 2:
            continue
        lo = min(values)
        hi = max(values)
        if lo == hi:
            return f"{field}: all values {lo:.4g}"
        buckets = [0] * 8
        for value in values:
            index = min(7, int(((value - lo) / (hi - lo)) * 8))
            buckets[index] += 1
        max_bucket = max(buckets) or 1
        bars = ["#" * max(1, round(count / max_bucket * 20)) if count else "." for count in buckets]
        return f"{field} {lo:.4g}..{hi:.4g}\n" + "\n".join(f"{i + 1}: {bar} {count}" for i, (bar, count) in enumerate(zip(bars, buckets)))
    return ""


def _data_stats_result(action: str, paths: list[str]) -> dict[str, Any]:
    blocks = []
    for path in paths:
        payload = filesystem.read_file(path)
        content = str(payload.get("content", ""))
        suffix = Path(path).suffix.lower()
        if suffix == ".json":
            stats = _json_stats(content)
        elif suffix == ".ndjson":
            lines = [line for line in content.splitlines() if line.strip()]
            stats = f"type: NDJSON\nrecords: {len(lines)}"
        else:
            stats = _csv_stats(content, "\t" if suffix == ".tsv" else ",")
        blocks.append(f"## {path}\n\n{stats}")
    return _result(action, "Data stats", "\n\n".join(blocks), paths)


def _ocr_result(action: str, paths: list[str]) -> dict[str, Any]:
    tesseract = shutil.which("tesseract")
    if not tesseract:
        return _result(action, "OCR unavailable", "OCR needs the `tesseract` executable on the server PATH. No image text was extracted.", paths, unavailable=True)
    blocks = []
    for path in paths:
        filesystem.read_raw(path)
        completed = subprocess.run(
            [tesseract, path, "stdout"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            timeout=20,
            check=False,
        )
        if completed.returncode == 0:
            text = completed.stdout.strip() or "(no text detected)"
        else:
            text = (completed.stderr or completed.stdout or "OCR failed").strip()
        blocks.append(f"## {path}\n\n{text[:12_000]}")
    return _result(action, "OCR result", "\n\n".join(blocks), paths)


def run_drop_action(payload: dict[str, Any]) -> tuple[dict[str, Any], int]:
    action = str(payload.get("action") or payload.get("id") or "").strip()
    paths = _payload_paths(payload)
    if not action:
        return {"ok": False, "error": "action is required"}, 400
    if not paths:
        return {"ok": False, "error": "path is required"}, 400
    try:
        if action == "server-info":
            return _info_result(action, paths), 200
        if action == "server-head":
            return _head_result(action, paths), 200
        if action == "server-log-errors":
            return _log_errors_result(action, paths), 200
        if action == "server-data-stats":
            return _data_stats_result(action, paths), 200
        if action == "server-ocr":
            return _ocr_result(action, paths), 200
    except filesystem.FilesystemError as exc:
        return {"ok": False, "action": action, "paths": paths, "error": str(exc)}, int(exc.status)
    except (OSError, subprocess.SubprocessError, json.JSONDecodeError, csv.Error, ValueError) as exc:
        return {"ok": False, "action": action, "paths": paths, "error": str(exc)}, 500
    return {"ok": False, "action": action, "paths": paths, "error": f"unknown action: {action}"}, 400

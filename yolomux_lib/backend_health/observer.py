...
class ObservationCycle:
    ...
    # absence_expected, demand_scoped). Recovery, process identity, expected absence, and the
    # static lifetime class are part of the STABLE SIGNATURE because each can change while the
    # visible state remains the same.
    signature: tuple[tuple[str, str, str, str, int, str, bool, bool], ...]
...
class BackendHealthObserver:
    ...
    def __init__(
        ...
        *,
        ...
        self._published_signature: tuple[tuple[str, str, str, str, int, str, bool, bool], ...] | None = None
    ...
    def observe_once(self) -> ObservationCycle:
        ...
        observed_demand_scoped: dict[str, bool] = {}
        ...
        for row in snapshot.rows:
            ...
            observed_demand_scoped[row.service] = row.fields.get("demand_started") is True
        ...
        with self._lock:
            published_identities = {
                ...
                for name, _state, _reason, _recovery, pid, start_identity, _expected, _demand_scoped in (self._published_signature or ())
            }
            published_expected_absence = {
                name: expected
                for name, _state, _reason, _recovery, _pid, _start_identity, expected, _demand_scoped in (self._published_signature or ())
            }
            published_demand_scoped = {
                name: demand_scoped
                for name, _state, _reason, _recovery, _pid, _start_identity, _expected, demand_scoped in (self._published_signature or ())
            }
            accepted_identities = {
                name: (
            ...
            accepted_demand_scoped = {
                name: (
                    observed_demand_scoped.get(name, False)
                    if observed.get(name) == health
                    else published_demand_scoped.get(name, False)
                )
                for name, health in accepted.items()
            }
            signature = tuple(
                sorted(
                    (
                        ...
                        accepted_demand_scoped.get(name, False),
                    )
    ...
    def _publish_change(
        ...
        demand_scoped: Mapping[str, bool],
        signature: tuple[tuple[str, str, str, str, int, str, bool, bool], ...],
        ...
        observations = tuple(
            ResourceObservation(
                ...
                demand_scoped=demand_scoped.get(name, False),

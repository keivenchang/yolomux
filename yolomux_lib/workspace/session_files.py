...
from ..tmux.sessions import codex_transcript_family_paths
from ..tmux.sessions import CODEX_TRANSCRIPT_SCAN_LIMIT
...
def load_transcript_scan_state(cache_key: tuple[Any, ...], path: Path, new_state: Callable[[], dict[str, Any]]) -> dict[str, Any] | None:
    ...
    try:
        with file_lock(cache_path, dir_mode=0o700):
            record = read_json_file(cache_path, {}, exceptions=())
    ...
    try:
        with file_lock(cache_path, dir_mode=0o700):
            if cache_path.exists():
                existing = read_json_file(cache_path, {}, exceptions=())

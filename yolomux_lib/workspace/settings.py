...
HOVER_SHOW_DELAY_MS = 1300
HOVER_HIDE_DELAY_MS = 120
RETIRED_HOVER_TIMING_KEYS = ("menu_hover_open_delay_ms", "tab_popover_show_delay_ms", "tab_popover_follow_delay_ms")
...
DEFAULT_SETTINGS: dict[str, Any] = {
    ...
    "performance": {
        ...
        "popover_show_delay_ms": 1300,
        "popover_hide_delay_ms": 120,
...
STALE_DEFAULT_MIGRATIONS: dict[tuple[str, str], Any] = {
    ...
    ("performance", "auto_approve_interval_seconds"): 2.0,
...
SETTING_LIMITS: dict[tuple[str, str], tuple[float, float]] = {
    ...
    ("performance", "popover_hide_delay_ms"): (0, 1000),
    ("performance", "remote_resize_delay_ms"): (50, 2000),
    ("performance", "session_files_max_workers"): (1, 8),
    ("performance", "auto_approve_interval_seconds"): (0.1, 4),
...
SETTING_COMMENTS: dict[tuple[str, str], str] = {
    ...
    ("file_explorer", "index_exclude_paths"): "Quick Open exclusions, one rule per line. A bare directory name excludes that exact name anywhere. A plain absolute or home-relative path excludes that subtree. glob:<relative glob> matches paths below every indexed root (for example glob:**/.uploads/**). regex:<regular expression> matches each root-relative POSIX path (for example regex:(^|/)target(?:/|$)). Invalid regex rules are ignored.",
    ("file_explorer", "companion_dirs"): "Extra directories always included when computing per-session repo status (branch, dirty count, ahead/behind), one path per line. Useful for companion repos that sit alongside your session workdirs but are rarely the active pane cwd — e.g. ~/nvidia/frontend-crates.",
    ...
    ("performance", "popover_show_delay_ms"): "Milliseconds, 0-3000. Shared delay before every hover surface opens.",
    ("performance", "popover_hide_delay_ms"): "Milliseconds, 0-1000. Shared delay before every hover surface closes or follows another target.",
    ...
    ("performance", "session_files_max_workers"): "Workers, 1-8. Maximum concurrent cold session-files rebuilds; two is the measured default that avoids transcript, Git, and disk contention.",
    ("performance", "auto_approve_interval_seconds"): "Seconds, 0.1-4. Fast poll interval for working or changing YOLO panes; quiet panes back off to 4 seconds.",
...
SETTING_GUI_SECTIONS: dict[tuple[str, str], str] = {
    ...
    ("appearance", "terminal_theme"): "Colors",
    ("appearance", "editor_dark_color_scheme"): "Colors",
    ("appearance", "editor_light_color_scheme"): "Colors",
    ("appearance", "editor_cursor_color"): "Colors",
    ("appearance", "editor_cursor_style"): "Colors",
    ("appearance", "active_color"): "Colors",
    ("appearance", "separator_color"): "Colors",
    ("appearance", "pane_ring_opacity"): "Colors",
    ("appearance", "inactive_pane_opacity"): "Colors",
    ("appearance", "ui_font_size"): "Sizes",
    ("appearance", "global_font_size"): "Sizes",
    ("appearance", "sync_font_sizes"): "Sizes",
    ("appearance", "terminal_font_size"): "Sizes",
    ("appearance", "file_explorer_font_size"): "Sizes",
    ("appearance", "tab_width"): "UI sizes",
    ("appearance", "pane_spacing"): "UI sizes",
    ("appearance", "max_tabs_per_pane"): "UI sizes",
    ("appearance", "tmux_status_bar"): "Terminal",
    ("terminal_editor", "scrollback"): "Terminal",
    ("appearance", "editor_font_size"): "Sizes",
    ("appearance", "preview_font_size"): "Sizes",
    ("terminal_editor", "word_wrap"): "Editor options",
    ("terminal_editor", "line_numbers"): "Editor options",
    ("editor", "autosave"): "Editor options",
    ("editor", "autosave_delay_seconds"): "Editor options",
    ("editor", "blame_all_lines"): "Editor options",
    ("editor", "trim_trailing_whitespace_on_save"): "Editor options",
    ("editor", "ensure_final_newline_on_save"): "Editor options",
    ("general", "default_layout"): "General",
    ("appearance", "date_time_hour_cycle"): "General",
...
SETTING_GUI_SECTION_LOCALE_KEYS = {
    "Appearance": "pref.section.sizes",
    "Colors": "pref.section.colors",
    "Sizes": "pref.section.sizes",
    ...
    "Editor options": "pref.section.editor_options",
    "Terminal": "pref.section.terminal",
    "UI sizes": "pref.section.ui_sizes",
...
def sanitize_settings(raw: Any, coerced: list[str] | None = None) -> dict[str, Any]:
    ...
    legacy_performance = source.get("performance") if isinstance(source.get("performance"), dict) else {}
    if any(key in legacy_performance for key in RETIRED_HOVER_TIMING_KEYS):
        sanitized["performance"]["popover_show_delay_ms"] = HOVER_SHOW_DELAY_MS
        sanitized["performance"]["popover_hide_delay_ms"] = HOVER_HIDE_DELAY_MS
    return sanitized

...
- The pane info line shows one button per tmux window (`0:bash`, `1:codex`, ...); clicking a button switches that session to the matching tmux window, and the path/repo metadata follows that selected window instead of inheriting touched repos from another window in the same session. Each Claude/Codex window has its own path/branch/git state; a plain shell window has cwd/path but no agent state. The pane toolbar shows transcripts (`Tx`), asks for an AI summary (`AI`), and opens the event log (`Log`).

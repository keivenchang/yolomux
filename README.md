...
- In Finder, right-click or long-press one Git-backed directory and choose `Diff repo` to open bounded newest-first repository history in a normal Generic Pane tab. Expand one or more commits to read the full message and changed-file tree with status and `+N/-N` or binary metadata; selecting a file opens another instance of the current Editor/Preview/Diff tab with Diff selected for that commit's exact parent-to-commit refs, immutable Preview content, and read-only Edit. A file row instead offers `Edit in new tab`, `Preview in new tab`, and `Diff in new tab`; all three activate one canonical working-tree Editor/Preview/Diff tab and select only its mode, so dirty content and pane placement remain intact.

...
- Hover a tmux tab to see each Claude/Codex window in that session near the top of the popover, with working agents first, idle durations from recent activity, and path/branch/git metadata under the matching AI window when windows differ.
...
- The pane Info Bar shows one button per tmux window (`0:bash`, `1:codex`, ...); clicking a button switches that session to the matching tmux window, and the path/repo metadata follows that selected window instead of inheriting touched repos from another window in the same session. Each Claude/Codex window has its own path/branch/git state; a plain shell window has cwd/path but no agent state. The pane toolbar shows transcripts (`Tx`), asks for an AI summary (`AI`), and opens the event log (`Log`).
...
- File -> `YO!stats` opens debug performance stats. `API/SSE` shows recent request/event text and counters; `Graph` shows line-series history split by unit: API/SSE count per second, latency, bandwidth per second, and CPU each get their own Y axis with concrete unit labels and X-axis time ticks. The CPU chart overlays `yolomux.py` CPU and system average CPU on a fixed 0-100% scale. The graph has 1, 5, and 10 second scale buttons, selectable ranges from last minute through 24 hours, plus a meta row with `yolomux.py uptime`, PID, RSS, server sequence, and total upload/download MB. The 8 hour and 16 hour range buttons appear only after retained history reaches those ages. Graph history is remembered in the server for browser refreshes, fetched incrementally by sequence, and bounded to 24 hours with data older than one hour rolled into ten-second buckets.
...
- A third `Tabber` mode lists open tabs and each tmux session's windows, sorted by recent activity. Click any non-arrow part of a green session row such as `2 #10731...` to focus that tmux tab, click the disclosure arrow to collapse/expand it, and click a window row to switch tmux windows. Agent touched-path rows are cached by the server, attach only under the matching agent window, and keep their recency text visible while the tree text truncates first.

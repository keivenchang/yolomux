...
- The Finder root toggle is `No Sync` / `Sync`. In Sync mode, Finder follows the focused session's affected repos: if a session touches multiple nearby repos, Finder opens their common parent, expands each touched repo path, and highlights affected repo roots plus changed descendant directories; with no changed-file payload yet, or with a stale same-session payload from a different repo, it falls back to the session's git root, and a fresh session with no cwd opens home.

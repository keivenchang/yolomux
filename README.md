...
- File -> `YO!info` opens the grouped relationship tree over Tab, AI, path, branch, PR, and Linear metadata. It renders one record per Tab x AI x Path x Branch association, keeps unassigned branch-inventory rows visible, and lets the grouping order switch between presets such as `Tab > Path`, `Path > Branch`, `Linear > PR`, and `PR > Branch` or custom per-level selectors.

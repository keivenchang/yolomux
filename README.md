...
- File -> `Finder/Differ/Tabber` opens each independent file-surface tab. On desktop and wide landscape tablets, newly opened items join the narrow left home column; once moved, selecting the same item activates it where it is. On narrow touch screens, File opens only the selected one in the full-width pane. Filesystem permission failures are reported in Finder instead of terminating the server request. Quick Search is `Mod+P`; it hides clean deleted file tabs, keeps dirty buffers reachable when their backing path is missing, and restores clean tabs when the file reappears.
...
- Tabber lists open tabs and tmux sub-windows by recent activity. `Mod+B` hides all open Finder/Differ/Tabber tabs or restores the default left home column. The top-bar language picker changes the live UI language.

...
- Use the platform app modifier (`⌘` on Mac, `Ctrl` on PC) for app shortcuts: app-modifier+K or Shift+app-modifier+P opens the command palette, and app-modifier+P opens file quick-open. On Mac, Ctrl stays reserved for tmux. The command palette searches tabs, menu actions, and settings from one centered popup; file quick-open searches files under the active session cwd/repo with recently opened files first. The `Tabs` menu also has its own fuzzy search box at the top.

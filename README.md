...
- Quick Search ranks filename matches ahead of path-only fuzzy matches, so numbered files such as `DOIT.53.md` and `DOIT.51.md` stay grouped. Image hits render as Cursor-style references such as `[Image #1] '/home/user/project/screenshot.png'`, while keeping the absolute path visible and searchable.

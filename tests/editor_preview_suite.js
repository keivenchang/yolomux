...
async function runEditorPreviewSuite({shardIndex = 0, shardCount = 1} = {}) {
  ...
  test('touch terminals provide one shared smart-key transport accessory', () => {
    ...
    assert.ok(/function terminalMobileAccessoryPositionStyle[\s\S]*inset-block-end:auto[\s\S]*function syncTerminalMobileAccessoryState[\s\S]*launcher\.style\.insetBlockEnd = launcherBlock \? 'auto' : ''/.test(source), 'a dragged launcher releases its bottom anchor instead of stretching down the terminal');

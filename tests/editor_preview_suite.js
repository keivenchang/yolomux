...
async function runEditorPreviewSuite({shardIndex = 0, shardCount = 1} = {}) {
  ...
  test('touch terminals provide one shared smart-key transport accessory', () => {
    ...
    assert.ok(/terminalMobileAccessoryModifierActions = Object\.freeze\(\['ctrl', 'alt', 'shift', 'cmd'\]\)[\s\S]*terminalMobileAccessoryPrimaryActions = Object\.freeze\(\['tab', 'tmux-prefix', 'backspace', 'more'\]\)[\s\S]*terminalMobileAccessorySideActions = Object\.freeze\(\['escape', 'ctrl', 'shift'\]\)[\s\S]*terminalMobileAccessoryBottomActions = Object\.freeze\(\['alt', 'cmd'\]\)/.test(source), 'the smart-key side column is Esc/Ctrl/Shift while Alt/Cmd share the primary bottom row');
    assert.ok(/const terminalMobileAccessoryMoreKeyDefs[\s\S]*action: 'command-p'[\s\S]*action: 'home'[\s\S]*action: 'ctrl-r'/.test(source) && /const terminalMobileAccessoryMoreActions = Object\.freeze\(\[[\s\S]*'interrupt'/.test(source) && !/const terminalMobileAccessoryMoreKeyDefs[\s\S]{0,500}action: 'command-v'/.test(source), 'touch terminal overflow reuses direct Ctrl-C with Cmd-P and secondary navigation while Paste stays visible in the primary grid');
    ...
    assert.ok(/mobile-terminal-key-side[\s\S]*data-terminal-mobile-key="escape"[\s\S]*data-terminal-mobile-key="ctrl"[\s\S]*data-terminal-mobile-key="shift"[\s\S]*mobile-terminal-keyrow-shell[\s\S]*data-terminal-mobile-key="tab"[\s\S]*data-terminal-mobile-key="tmux-prefix"[\s\S]*data-terminal-mobile-key="backspace"[\s\S]*mobile-terminal-keyrow-bottom[\s\S]*data-terminal-mobile-key="alt"[\s\S]*data-terminal-mobile-key="cmd"[\s\S]*mobile-terminal-keyrow--more[\s\S]*data-terminal-mobile-key="interrupt"/.test(openKeyboardHtml), 'rendered mobile keyboard keeps Esc/Ctrl/Shift in the side column, puts Alt/Cmd on its bottom row, and moves direct Ctrl-C into overflow');
    ...
    assert.ok(source.includes("const terminalMobileAccessoryPrimaryActions = Object.freeze(['tab', 'tmux-prefix', 'backspace', 'more']);") && source.includes("const terminalMobileAccessorySideActions = Object.freeze(['escape', 'ctrl', 'shift']);") && source.includes("const terminalMobileAccessoryBottomActions = Object.freeze(['alt', 'cmd']);") && source.includes("'shift-tab', 'interrupt', 'ctrl-d'"), 'the touch palette keeps only Esc/Ctrl/Shift in its side column, Alt/Cmd in one bottom row, and direct Ctrl-C in overflow without a duplicate definition');
    ...
    assert.ok(/\.mobile-terminal-key-side\s*\{[\s\S]*grid-template-rows:\s*repeat\(3, 36px\)[\s\S]*\.mobile-terminal-keyrow-bottom\s*\{[\s\S]*grid-column:\s*1 \/ -1[\s\S]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)[\s\S]*\.mobile-terminal-key-dpad\s*\{[\s\S]*grid-template-columns: repeat\(5, 36px\)/.test(css), 'the touch key palette has a three-key side column, a full-width Alt/Cmd bottom row, and the five-column D-pad');
    assert.ok(/\.mobile-terminal-key-launcher\s*\{[\s\S]*user-select:\s*none[\s\S]*\.mobile-terminal-keybar\s*\{[\s\S]*inset-block-end:\s*calc\([\s\S]*padding:\s*var\(--space-2\)[\s\S]*user-select:\s*none[\s\S]*\.mobile-terminal-keyrow-shell > \.mobile-terminal-key--more\s*\{[\s\S]*align-self:\s*start/.test(css), 'the non-selectable touch palette opens above its launcher and keeps More in the shared top-right shell');
  });

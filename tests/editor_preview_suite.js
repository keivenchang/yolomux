...
async function runEditorPreviewSuite({shardIndex = 0, shardCount = 1} = {}) {
  ...
  test('touch terminals provide one shared smart-key transport accessory', () => {
    ...
    assert.ok(/delegate\(panel, 'click', '\[data-terminal-mobile-close\]'[\s\S]*sendTerminalMobileAccessoryInput\(button\.dataset\.terminalMobileClose \|\| session, 'open'\)/.test(source) && /function sendTerminalMobileAccessoryInput[\s\S]*action === 'more' \|\| action === 'open'[\s\S]*toggleTerminalMobileAccessoryState\(session, action\)/.test(source), 'the X and both More buttons reuse the shared palette state owner');

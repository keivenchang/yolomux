...
async function runEditorPreviewSuite({shardIndex = 0, shardCount = 1} = {}) {
  ...
  test('touch terminals provide one shared smart-key transport accessory', () => {
    ...
    assert.ok(/terminalMobileAccessoryPrimaryActions = Object\.freeze\(\['escape', 'tab', 'tmux-prefix', 'more'\]\)[\s\S]*terminalMobileAccessorySideActions = Object\.freeze\(\['ctrl', 'interrupt'\]\)/.test(source), 'the smart-key utility column keeps Ctrl and direct Ctrl-C together below the drag handle');
    ...
    assert.ok(/\.mobile-terminal-key-launcher\s*\{[\s\S]*position:\s*absolute[\s\S]*\.mobile-terminal-keybar\s*\{[\s\S]*position:\s*absolute[\s\S]*grid-auto-rows:\s*max-content[\s\S]*\.mobile-terminal-key-side\s*\{[\s\S]*grid-template-rows:\s*repeat\(3, 36px\)[\s\S]*\.mobile-terminal-keyrow--primary\s*\{[\s\S]*grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)[\s\S]*\.mobile-terminal-keyrow-shell > \.mobile-terminal-key--more\s*\{[\s\S]*align-self:\s*start[\s\S]*\.mobile-terminal-keybar--more \.mobile-terminal-keyrow--more \.mobile-terminal-key--more\s*\{[\s\S]*grid-column:\s*4[\s\S]*grid-row:\s*1[\s\S]*\.mobile-terminal-key-dpad\s*\{[\s\S]*grid-template-columns: repeat\(5, 36px\)[\s\S]*mobile-terminal-key-dpad \.mobile-terminal-key--copy[\s\S]*grid-column:\s*1[\s\S]*mobile-terminal-key-dpad \.mobile-terminal-key--tmux-scroll-up[\s\S]*grid-column:\s*5[\s\S]*mobile-terminal-key--arrow-up/.test(css), 'the touch key palette stacks drag/Ctrl/Ctrl-C left of the D-pad, pins More to the top-right, and keeps Ctrl-B/Tab on the first page');
    assert.ok(/\.mobile-terminal-key-launcher\s*\{[\s\S]*user-select:\s*none[\s\S]*\.mobile-terminal-keybar\s*\{[\s\S]*inset-block-end:\s*calc\([\s\S]*user-select:\s*none[\s\S]*\.mobile-terminal-key-close\s*\{[\s\S]*position:\s*absolute[\s\S]*inset-inline-end:[\s\S]*\.mobile-terminal-key-side\s*\{[\s\S]*grid-column:\s*1[\s\S]*\.mobile-terminal-keyrow--primary\s*\{[\s\S]*grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)[\s\S]*\.mobile-terminal-keyrow-shell > \.mobile-terminal-key--more\s*\{[\s\S]*align-self:\s*start[\s\S]*\.mobile-terminal-keybar--more \.mobile-terminal-keyrow--more \.mobile-terminal-key--more\s*\{[\s\S]*grid-column:\s*4[\s\S]*grid-row:\s*1[\s\S]*\.mobile-terminal-key-dpad\s*\{[\s\S]*grid-template-columns: repeat\(5, 36px\)[\s\S]*mobile-terminal-key-dpad \.mobile-terminal-key--copy[\s\S]*grid-column:\s*1[\s\S]*mobile-terminal-key-dpad \.mobile-terminal-key--tmux-scroll-up[\s\S]*grid-column:\s*5[\s\S]*mobile-terminal-key--arrow-up/.test(css), 'the non-selectable touch palette opens immediately above its launcher, keeps its top-right close control, and preserves the utility-column/D-pad geometry');
  });

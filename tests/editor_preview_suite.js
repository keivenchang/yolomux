...
async function runEditorPreviewSuite({shardIndex = 0, shardCount = 1} = {}) {
  ...
  test('touch terminals provide one shared smart-key transport accessory', () => {
    ...
    assert.ok(/terminalMobileAccessoryModifierActions = Object\.freeze\(\['ctrl', 'alt', 'shift', 'cmd'\]\)[\s\S]*terminalMobileAccessoryPrimaryActions = Object\.freeze\(\['tab', 'tmux-prefix', 'backspace', 'more'\]\)[\s\S]*terminalMobileAccessoryCornerAction = 'escape'[\s\S]*terminalMobileAccessorySideActions = Object\.freeze\(\['ctrl', 'shift'\]\)[\s\S]*terminalMobileAccessoryDpadActions = Object\.freeze\([^\n]*'copy', 'command-v'[^\n]*'alt'/.test(source), 'the primary page keeps Esc in the corner, stacks Paste directly below Copy, and keeps Ctrl, Alt, then Cmd across the bottom-left row');
    ...
    assert.ok(/\.mobile-terminal-keybar\s*\{[\s\S]*width:\s*min\(18rem,[\s\S]*height:\s*min\(204px,[\s\S]*\.mobile-terminal-key-content\s*\{[\s\S]*overflow:\s*hidden[\s\S]*\.mobile-terminal-key-content--more\s*\{/.test(css), 'primary and More pages share one fixed non-scrolling 288x204 palette footprint');
    ...
    assert.ok([
      ...
      /\.mobile-terminal-key-side \.mobile-terminal-key--shift\s*\{\s*grid-row:\s*2[\s\S]*\.mobile-terminal-key-side \.mobile-terminal-key--ctrl\s*\{\s*grid-row:\s*3/,
      /\.mobile-terminal-keybar\s*>\s*\.mobile-terminal-key--close\s*\{[\s\S]*position:\s*absolute[\s\S]*inset-inline-end/,
      /\.mobile-terminal-key-content--more\s*\{[\s\S]*grid-template-columns:\s*repeat\(4, minmax\(0, 1fr\)\)/,
      /\.mobile-terminal-key-dpad\s*\{[\s\S]*grid-column:\s*2[\s\S]*grid-template-columns:\s*36px 32px 36px 32px 44px 36px/,
      /\.mobile-terminal-key-dpad \.mobile-terminal-key--interrupt\s*\{\s*grid-column:\s*6;\s*grid-row:\s*3/,
      /\.mobile-terminal-key-dpad \.mobile-terminal-key--alt\s*\{\s*grid-column:\s*1;\s*grid-row:\s*3[\s\S]*\.mobile-terminal-key-dpad \.mobile-terminal-key--cmd\s*\{\s*grid-column:\s*2;\s*grid-row:\s*3/,
      /\.mobile-terminal-key--enter\s*\{\s*grid-column:\s*5;\s*grid-row:\s*2[\s\S]*\.mobile-terminal-key--arrow-right\s*\{\s*grid-column:\s*4;\s*grid-row:\s*2/,

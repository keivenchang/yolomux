...
async function runEditorPreviewSuite() {
  ...
  test('Finder/Differ/Tabber recency brightness and pulse apply in Ago and Date modes', () => {
    ...
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.status-indicator--working::before,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--working::before\s*\{[\s\S]*background:\s*var\(--subwindow-status-glyph-border-color\)[\s\S]*clip-path:\s*polygon/.test(paneTabsCss), 'working sub-window status draws the outer border of a centered CSS play triangle');
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.status-indicator--working::after,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--working::after\s*\{[\s\S]*background:\s*var\(--subwindow-status-glyph-fill\)[\s\S]*clip-path:\s*polygon/.test(paneTabsCss), 'working sub-window status fills the bordered triangle with the shared green token');
    ...
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.status-indicator--cooldown::before,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--cooldown::after\s*\{[\s\S]*--subwindow-status-pause-bar-offset:\s*calc\(var\(--agent-status-ball-size\) \* var\(--subwindow-status-glyph-scale\) \* 0\.31\)[\s\S]*inline-size:\s*calc\(var\(--agent-status-ball-size\) \* var\(--subwindow-status-glyph-scale\) \* 0\.16\)[\s\S]*background:\s*var\(--subwindow-status-glyph-fill, currentColor\)[\s\S]*filter:\s*none/.test(paneTabsCss), 'cooldown sub-window status uses widened paired pause bars and the same no-glow opacity mechanism');
    ...
    assert.ok(/\.status-indicator--attention\s*\{[^}]*--agent-status-ball-fill:\s*var\(--bad\)[^}]*--attention-ring-rgb:\s*255 51 71/.test(sessionsCss), 'attention markers own the red aggregate-ball fill instead of falling back to transparent text color');

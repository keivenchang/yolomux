...
async function runEditorPreviewSuite() {
  ...
  test('coarse-pointer tablets retain menus while phones compact the topbar', () => {
    ...
    assert.ok(/app-topbar-touch-compact\.app-vw-lte-1200 #latencyMeter,[\s\S]*?app-vw-lte-1100 \.topbar-nav,[\s\S]*?#status[\s\S]*?\.topbar-activity--mobile-count-balls \.topbar-activity-count\.active[\s\S]*?inline-grid/.test(css), 'the priority ladder hides ping, then navigation, while retaining active AI tones as compact count-in-ball indicators');
    assert.ok(/app-topbar-menu-compact \.actions > :not\(#refreshMeta\):not\(\.topbar-activity--mobile-count-balls\)\s*\{[\s\S]*?display:\s*none[\s\S]*?app-topbar-menu-compact #refreshMeta\s*\{[\s\S]*?display:\s*inline-grid/.test(css), 'shared compact chrome keeps Refresh and active work balls while lower-priority top-right buttons yield before menus');
    ...
    assert.ok(/\.actions > \.topbar-activity--mobile-count-balls\s*\{[\s\S]*?display:\s*inline-flex[\s\S]*?overflow:\s*visible/.test(css) && /function topbarActivityUsesActionsRail\(\)[\s\S]*topbarActivityUsesMobileCountBalls\(\)/.test(layoutSource) && /function syncTopbarActivityPlacement\(\)[\s\S]*topbarActivityUsesActionsRail\(\)[\s\S]*actions\.insertBefore\(activity, refresh \|\| null\)[\s\S]*normalHost\.append\(activity\)/.test(layoutSource), 'compact activity circles move into the actions rail immediately before Refresh at the shared compact-activity threshold and return to their normal host after expansion');
    assert.ok(/const topbarActionCapacityHideOrder = Object\.freeze\(\[[\s\S]*?'latencyMeter',[\s\S]*?'logoutButton',[\s\S]*?'notifyToggle',[\s\S]*?\]\)/.test(layoutSource) && /function syncTopbarActionCapacity\(\)[\s\S]*?classList\.remove\('topbar-action-capacity-hidden'\)[\s\S]*?topbarActionCapacityFits\(\)[\s\S]*?classList\.add\('topbar-action-capacity-hidden'\)/.test(layoutSource), 'the action rail resets then removes only low-priority latency, Exit, and notification icons until compact activity plus the menu/search row fit');
    const coreSource = fs.readFileSync('static_src/js/yolomux/10_core_utils.js', 'utf8');
  ...
  await testAsync('YO!stats retries timed-out cold starts quickly, then switches to steady polling after its first sample', async () => {
    ...
    const coldInterval = [...timeouts.entries()].map(([id, timer]) => ({id, ...timer})).filter(timer => timer.ms === 2000 && !clearedTimeouts.has(timer.id)).at(-1);
    await flushAsyncWork();
    ...
    await flushAsyncWork();
    await flushAsyncWork();
    const steadyInterval = [...timeouts.entries()].map(([id, timer]) => ({id, ...timer})).filter(timer => timer.ms === 30000 && !clearedTimeouts.has(timer.id)).at(-1);

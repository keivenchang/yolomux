...
async function runEditorPreviewSuite() {
  ...
  test('Finder/Differ/Tabber recency brightness and pulse apply in Ago and Date modes', () => {
    ...
    assert.ok(/\.status-indicator--dot\.heartbeat-pulse\s*\{[\s\S]*--attention-pulse-brightness-rest:\s*1[\s\S]*--attention-pulse-brightness-peak:\s*1\.34[\s\S]*--attention-ring-peak-glow-alpha:\s*0\.68/.test(sessionsCss), 'pulsing dots keep their canonical resting fill while the shared glow channel makes the pulse visible on active accent backgrounds');
    ...
    assert.equal(/background:\s*currentColor/.test(dotBlock), false, 'status balls are the old glyph dot, not a CSS-filled disc');
    ...
    assert.ok(/\.tmux-window-button \.agent-window-status-dot,\s*\.session-agent-row \.agent-window-status-dot,\s*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\s*\{[\s\S]*--subwindow-status-attention-fill:\s*var\(--danger-strong\)[\s\S]*--subwindow-status-cooldown-fill:\s*var\(--agent-status-cooldown\)[\s\S]*--subwindow-status-acknowledged-fill:[\s\S]*--subwindow-status-glyph-scale:\s*0\.8[\s\S]*--subwindow-status-glyph-fill:\s*currentColor[\s\S]*--subwindow-status-glyph-glow-rgb:[\s\S]*--subwindow-status-glyph-border-color:[\s\S]*--subwindow-status-glyph-outline-filter:[\s\S]*drop-shadow[\s\S]*--subwindow-status-glyph-base-filter:[\s\S]*border-radius:\s*0[\s\S]*color:\s*transparent/.test(paneTabsCss), 'tmux sub-window, popover row, and Tabber window status dots hide the shared circle glyph with a flat 80% play/stop/pause glyph plus scoped stop-red, pause-yellow, acknowledged-gray, and glow-owner fills');
    ...
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.agent-window-status-dot--acknowledged,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.agent-window-status-dot--acknowledged\s*\{[\s\S]*--subwindow-status-glyph-fill:\s*var\(--subwindow-status-acknowledged-fill\)/.test(paneTabsCss), 'acknowledged sub-window glyphs turn gray without changing the shared 80% size');
    assert.ok(/\.tmux-window-bar \.tmux-window-button\.active \.agent-window-status-dot\.status-indicator--cooldown,[\s\S]*--subwindow-status-glyph-fill:\s*var\(--subwindow-status-cooldown-fill\)[\s\S]*\.tmux-window-bar \.tmux-window-button\.active \.agent-window-status-dot\.agent-window-status-dot--acknowledged,[\s\S]*--subwindow-status-glyph-fill:\s*var\(--subwindow-status-acknowledged-fill\)/.test(paneTabsCss), 'acknowledged gray wins over the active tmux sub-window status color');
    assert.ok(/const agentWindowActivityAcknowledgeDelayMs\s*=\s*700;/.test(activitySource), 'sub-window acknowledgement turns gray after the 700ms interaction hold');
    ...
    assert.ok(/function agentWindowActivityIconHtml\(agentKey, state, idleSeconds, options = \{\}\)[\s\S]*const subwindowGlyphPulse = options\.subwindowGlyphPulse === true \|\| \(options\.subwindowGlyphPulse !== false && statusOnly !== true\)[\s\S]*agentWindowStatusDotHtml\(item, \{statusTones: options\.statusTones, animate: options\.animate !== false, subwindowGlyphPulse\}\)/.test(activitySource), 'sub-window rows opt into the cheap glyph pulse while status-only Tab aggregate balls stay on the normal dot path');
    ...
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.agent-window-status-dot--subwindow-pulse::before,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--cooldown\.agent-window-status-dot--subwindow-pulse::after\s*\{[\s\S]*animation-name:\s*subwindow-status-glyph-pulse/.test(paneTabsCss), 'sub-window pulse animates the glyph pseudo-elements instead of the status-dot element');
    assert.ok(/@keyframes subwindow-status-glyph-pulse\s*\{[\s\S]*opacity:\s*1[\s\S]*scale\(0\.92\)[\s\S]*drop-shadow\(0 0 3px rgb\(var\(--subwindow-status-glyph-glow-rgb\) \/ 0\.36\)\)[\s\S]*opacity:\s*1[\s\S]*scale\(1\.08\)[\s\S]*drop-shadow\(0 0 8px rgb\(var\(--subwindow-status-glyph-glow-rgb\) \/ 0\.82\)\)/.test(paneTabsCss), 'sub-window glyph pulse scales and glows without dimming its vibrant fill');
    assert.equal(/@keyframes subwindow-status-glyph-pulse\s*\{[\s\S]*opacity:\s*0\./.test(paneTabsCss), false, 'sub-window glyph pulse never lowers opacity');
    assert.ok(/--subwindow-status-cooldown-outline-filter:[\s\S]*drop-shadow\(1px 0 0 var\(--subwindow-status-cooldown-outline-color\)\)[\s\S]*\.tmux-window-bar \.tmux-window-button\.active \.agent-window-status-dot,[\s\S]*--subwindow-status-cooldown-outline-color:\s*color-mix\(in srgb, var\(--active-control-bg\) 22%, #000 78%\)/.test(paneTabsCss), 'cooldown pause bars use a scoped contrasting outline, with an active-button dark outline that does not alter other glyphs');
    assert.ok(/\.tmux-window-button \.agent-window-status-dot::before,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--cooldown::after\s*\{[\s\S]*transform:\s*translate\(-50%, -50%\)/.test(paneTabsCss), 'sub-window glyph pseudo-elements use the centered base transform on all sub-window surfaces');
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.status-indicator--working::before,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--working::before\s*\{[\s\S]*border-inline-start:[\s\S]*solid var\(--subwindow-status-glyph-fill, currentColor\)/.test(paneTabsCss), 'working sub-window status uses a centered CSS play triangle from the shared glyph-fill owner');
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.status-indicator--attention::before,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--attention::before\s*\{[\s\S]*inline-size:\s*calc\(var\(--agent-status-ball-size\) \* var\(--subwindow-status-glyph-scale\)\)[\s\S]*background:\s*var\(--subwindow-status-glyph-fill, currentColor\)/.test(paneTabsCss), 'attention sub-window status uses a centered solid CSS stop square from the shared glyph-fill owner');
    ...
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.status-indicator--cooldown::before,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--cooldown::after\s*\{[\s\S]*--subwindow-status-pause-bar-offset:\s*calc\(var\(--agent-status-ball-size\) \* var\(--subwindow-status-glyph-scale\) \* 0\.31\)[\s\S]*inline-size:\s*calc\(var\(--agent-status-ball-size\) \* var\(--subwindow-status-glyph-scale\) \* 0\.16\)[\s\S]*background:\s*var\(--subwindow-status-glyph-fill, currentColor\)[\s\S]*filter:\s*var\(--subwindow-status-glyph-base-filter\)/.test(paneTabsCss), 'cooldown sub-window status uses widened paired pause bars with a per-bar outline from the shared glyph-fill owner');
    ...
    assert.ok(/\.tmux-window-bar \.tmux-window-button\.active \.agent-window-status-dot,\s*\.session-agent-window-block > \.session-agent-row\.current \.agent-window-status-dot\s*\{[\s\S]*--subwindow-status-glyph-border-color:\s*var\(--subwindow-status-glyph-border-color-active\)[\s\S]*--subwindow-status-glyph-outline-filter:[\s\S]*drop-shadow\(1px 0 0 var\(--subwindow-status-glyph-border-color\)/.test(paneTabsCss), 'active/current sub-window glyphs keep tone fill and use pressed-button text only as the outline');
    ...
    assert.ok(/function agentWindowStatusToneForItem\(item\)[\s\S]*agentWindowActivityTone\(item\.state\)[\s\S]*\['attention', 'cooldown', STATE_KEY\.working\]\.includes\(tone\)/.test(activitySource), 'visible agent-window status tones have one shared classifier for sub-window glyphs and aggregate segments');
    assert.ok(/function agentWindowStatusDotHtml\(item, options = \{\}\)[\s\S]*const tone = agentWindowStatusToneForItem\(item\)[\s\S]*agentWindowStatusToneOrder\(options\.statusTones \|\| \[tone\]\)[\s\S]*agent-window-status-dot--segmented[\s\S]*agent-window-status-dot--tone-/.test(activitySource), 'the shared status dot renderer owns segmented multi-tone tab balls');
    assert.ok(/function sessionStatusAgentWindowSummaryForTab\(session, info, payload = autoApproveStates\.get\(session\)\)[\s\S]*agentWindowStatusToneForItem\(item\)[\s\S]*byTone\.set\(tone, \{agent, item\}\)[\s\S]*item\.acknowledged !== true[\s\S]*selected = \{agent, item\}/.test(popoverSource), 'session tab aggregate segments include every visible sub-window tone while acknowledged windows do not broaden dominant selection');
    ...
    assert.ok(/\.agent-window-activity \.agent-window-status-dot--acknowledged\s*\{[\s\S]*font-size:\s*var\(--agent-status-ball-size-base\)/.test(sessionsCss), 'acknowledged red/yellow sub-window glyphs keep the base font size so the shared 80% acknowledged glyph scale is not multiplied twice');
    ...
    assert.ok(/\.status-indicator--cooldown\s*\{[^}]*color:\s*var\(--agent-status-cooldown\)[^}]*--attention-ring-rgb:\s*var\(--agent-status-cooldown-rgb\)/.test(sessionsCss), 'cooldown markers use the vibrant agent yellow and matching glow channel');
    ...
    assert.ok(/\.status-indicator--attention\s*\{[^}]*--attention-ring-rgb:\s*255 51 71/.test(sessionsCss), 'attention dot glow is red');
  ...
  test('t@6404', () => {
    ...
    assert.equal(acknowledgedStopped.acknowledged, true, 'acknowledged sticky yellow markers are tagged for the gray 80% visual state');
    ...
    assert.equal(acknowledgedWindowStopped.acknowledged, true, 'the acknowledged tmux sub-window marker is tagged for the gray 80% visual state');
    api.setWorkflowTransitionGlowSecondsForTest(60);
    ...
    assert.equal(acknowledgedAttention.acknowledged, true, 'acknowledged red attention is tagged for the gray 80% visual state');
    ...
    assert.ok(/class="tab tmux-window-button active"[\s\S]*data-window-index="0"[\s\S]*0:claude[\s\S]*agent-window-activity-icon--attention[\s\S]*status-indicator--attention/.test(activeAskWindowBarHtml), 'active 0:claude attention window button renders the red shared attention dot, not an idle/current white dot');
    api.setAutoApproveStateForTest('1', {agent_windows: [
      {kind: 'codex', state: 'working', window_index: 3, last_active_ts: nowSeconds, window_label: '3:codex'},
      {kind: 'codex', state: 'idle', window_index: 2, last_active_ts: nowSeconds - 120, idle_since: nowSeconds - 120, window_label: '2:codex'},
    ]});
    ...
    assert.ok(/0:claude[\s\S]*agent-window-activity-icon--working|agent-window-activity-icon--working[\s\S]*0:claude/.test(activityRunWindowBarHtml), 'Claude working from /api/activity drives the shared window-bar glyph');
    assert.ok(/1:codex[\s\S]*agent-window-activity-icon--working|agent-window-activity-icon--working[\s\S]*1:codex/.test(activityRunWindowBarHtml), 'Codex working from /api/activity drives the shared window-bar glyph');
    ...
    assert.ok(/\.tmux-window-button \.agent-window-activity \.agent-icon,\s*\.session-agent-row \.agent-window-activity \.agent-icon\s*\{[\s\S]*\.tmux-window-button \.agent-window-activity \.agent-icon svg,\s*\.session-agent-row \.agent-window-activity \.agent-icon svg\s*\{[\s\S]*width:\s*14px[\s\S]*height:\s*14px/.test(yoloCss), 'tmux sub-window and popover agent glyphs stay compact beside the canonical label');
    assert.ok(/\.tmux-window-button\.active \.agent-window-status-dot,\s*\.session-agent-window-block > \.session-agent-row\.current \.agent-window-status-dot\s*\{[\s\S]*--subwindow-status-glyph-border-color:\s*var\(--subwindow-status-glyph-border-color-active\)[\s\S]*drop-shadow\(1px 0 0 var\(--subwindow-status-glyph-border-color\)/.test(yoloCss), 'active tmux sub-window and current popover status glyphs keep tone fill with active-control text as the outline');
    ...
    assert.ok(/\.status-indicator--dot\.heartbeat-pulse\s*\{[\s\S]*--attention-pulse-brightness-rest:\s*1[\s\S]*--attention-pulse-brightness-peak:\s*1\.34[\s\S]*--attention-ring-peak-glow-alpha:\s*0\.68/.test(yoloCss), 'active tmux sub-window activity dots inherit the shared full-strength fill and glow pulse in the built CSS');
  ...
  test('t@6527', () => {
    ...
    assert.ok(/session-agent-kind[\s\S]*agent-icon codex[\s\S]*agent-window-status-dot(?=[^"]*agent-window-status-dot--transition-pulse)(?=[^"]*status-indicator--attention)[^"]*[\s\S]*1:codex/.test(agentPopover), 'attention popover row shows a static Codex symbol plus glyph-ready transition-pulsing red attention status before the label');
    ...
    assert.ok(/session-agent-activity-marker[\s\S]*agent-window-activity--status-only[\s\S]*agent-window-status-dot(?=[^"]*status-indicator--attention)(?=[^"]*agent-window-status-dot--segmented)(?=[^"]*agent-window-status-dot--attention-cooldown)(?=[^"]*agent-window-status-dot--tone-attention)(?=[^"]*agent-window-status-dot--tone-cooldown)/.test(redMarkerHtml), 'red+yellow tab status renders one segmented red/yellow ball without an agent icon');
    ...
    assert.ok(/session-agent-activity-marker[\s\S]*agent-window-activity--status-only[\s\S]*agent-window-status-dot(?=[^"]*status-indicator--cooldown)(?=[^"]*agent-window-status-dot--segmented)(?=[^"]*agent-window-status-dot--cooldown-working)(?=[^"]*agent-window-status-dot--tone-cooldown)(?=[^"]*agent-window-status-dot--tone-working)/.test(yellowMarkerHtml), 'yellow+green tab status renders one segmented yellow/green ball without an agent icon');
    ...
    assert.ok(/session-agent-activity-marker[\s\S]*agent-window-activity--status-only[\s\S]*agent-window-status-dot(?=[^"]*agent-window-status-dot--attention-cooldown-working)(?=[^"]*agent-window-status-dot--tone-attention)(?=[^"]*agent-window-status-dot--tone-cooldown)(?=[^"]*agent-window-status-dot--tone-working)/.test(triMarkerHtml), 'red+yellow+green tab status renders one tri-color ball without an agent icon');
  ...
  test('YO!stats graph renders server-shared agent activity and token area charts', () => {
    ...
    assert.ok(agentTokenChartHtml.includes('data-js-debug-moving-average="agentToken:') && agentTokenChartHtml.includes('data-js-debug-moving-average-samples="3"'), 'agent token chart overlays a short smoothing line');

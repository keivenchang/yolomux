...
async function runEditorPreviewSuite() {
  ...
  test('t@7149', () => {
    ...
    assert.ok(/addEventListener\('ready',[\s\S]{0,520}refreshAutoStatuses\(\{preferFresh: true\}\)\.catch/.test(source), 'client-events ready joins a fresh boot auto-status snapshot and re-fetches stale YO markers after reconnect');

...
async function runEditorPreviewSuite() {
  ...
  test('Finder/Differ/Tabber recency brightness and pulse apply in Ago and Date modes', () => {
    ...
    assert.ok(/\.tmux-window-button \.agent-window-status-dot,\s*\.session-agent-row \.agent-window-status-dot,\s*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\s*\{[\s\S]*--subwindow-status-attention-fill:\s*var\(--danger-strong\)[\s\S]*--subwindow-status-cooldown-fill:\s*#ffd633[\s\S]*--subwindow-status-acknowledged-fill:[\s\S]*--subwindow-status-glyph-scale:\s*0\.8[\s\S]*--subwindow-status-glyph-fill:\s*currentColor[\s\S]*--subwindow-status-glyph-border-color:[\s\S]*--subwindow-status-glyph-outline-filter:[\s\S]*drop-shadow[\s\S]*border-radius:\s*0[\s\S]*color:\s*transparent/.test(paneTabsCss), 'tmux sub-window, popover row, and Tabber window status dots hide the shared circle glyph with a flat 80% play/stop/pause glyph plus scoped stop-red, pause-yellow, and acknowledged-gray fills');
    ...
    assert.ok(/@keyframes subwindow-status-glyph-pulse\s*\{[\s\S]*opacity:\s*1[\s\S]*scale\(0\.92\)[\s\S]*opacity:\s*1[\s\S]*scale\(1\.04\)/.test(paneTabsCss), 'sub-window glyph pulse scales the shape without dimming its vibrant fill');
    ...
    assert.ok(/\.tmux-window-button \.agent-window-status-dot\.status-indicator--cooldown::before,[\s\S]*\.file-tree-row\.tabber-row\[data-tabber-type="window"\] \.tabber-window-label \.agent-window-status-dot\.status-indicator--cooldown::after\s*\{[\s\S]*--subwindow-status-pause-bar-offset:\s*calc\(var\(--agent-status-ball-size\) \* var\(--subwindow-status-glyph-scale\) \* 0\.31\)[\s\S]*inline-size:\s*calc\(var\(--agent-status-ball-size\) \* var\(--subwindow-status-glyph-scale\) \* 0\.16\)[\s\S]*background:\s*var\(--subwindow-status-glyph-fill, currentColor\)[\s\S]*filter:\s*var\(--subwindow-status-cooldown-outline-filter\)/.test(paneTabsCss), 'cooldown sub-window status uses widened paired pause bars with a per-bar outline from the shared glyph-fill owner');

...
async function runLayoutRestoreSuite() {
  ...
  testAsync('per-pane tab cap normalizes, refuses, and applies settings', async () => {
    ...
    assert.deepStrictEqual([...afterMinimize.panes.right.tabs], ['4', '2', '3'], 'mixed generic minimize moves only unpinned tabs to the target pane');
    ...
    assert.equal(api.statusKindForTest(), 'danger', 'all-pinned generic minimize reports a danger status');
    ...
    assert.equal(api.statusKindForTest(), 'danger', 'side-pane pinned minimize reports a danger status');

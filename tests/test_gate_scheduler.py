...
def _jobd_request(socket_path, action, **payload):
    envelope = rpc.new_envelope("jobd", action, {"action": action, **payload}, timeout_seconds=1.0)
...
def test_p6_failed_idle_pump_is_visible_and_the_broker_keeps_draining_work(tmp_path, monkeypatch):
    """One failed idle pump must not kill jobd or hide its reason from status."""

    broker = jobd.PersistentJobBroker(tmp_path / "jobd.sock", idle_seconds=60, workers=1)
    ...
    try:
        ...
        while time.monotonic() < deadline:
            result = _jobd_request(broker.socket_path, "result", job_id=job_id)
        ...
        final_status = _jobd_request(broker.socket_path, "status")

...
def assert_wait_for_jobd_product_uses_shared_bounded_cadence_until_ready(monkeypatch):
    ...
    class Client:
        def product(self, key, timeout=0.5):
            ...
            assert timeout == app_module.JOBD_PRODUCT_RPC_TIMEOUT_SECONDS
...
def assert_wait_for_jobd_product_retries_busy_within_the_existing_budget(monkeypatch):
    clock = [100.0]
    sleeps = []
    responses = iter([
...
def assert_wait_for_jobd_product_caps_rpc_at_outer_deadline(monkeypatch):
    clock = [100.0]
    rpc_timeouts = []
    ...
    result = app_module.wait_for_jobd_product(Client(), "product-key", 2, 0.14)

...
"""The gate live-server fixture must pin batchd for the whole fixture window, and release once.
...
``session_files_view`` to batchd (``TmuxWebtermApp.submit_session_files_job`` ->
``job_client.submit``). In production the local scheduler first pins batchd by taking the
scheduler lease (``handle_background_scheduler_started`` -> ``job_client.start_for_scheduler``),
so the batchd Unix socket is present and warm before any refresh worker submits.  The gate
fixture used to skip that pin entirely while its teardown still released the lease, so every
batchd interaction during the window was an on-demand cold start.  Under -n16 CPU contention
that cold start raced an absent socket (``FileNotFoundError`` at ``client.connect``) or timed
out, and the strict browser-journey gate caught the emitted ``local-service:batchd`` transport
...
from tests.gate_harness import pin_fixture_batchd_scheduler
from tests.gate_harness import prepare_fixture_http_app
from tests.gate_harness import stop_fixture_app_runtime


def test_gate_live_server_pins_batchd_scheduler_lease_for_the_window(gate_live_server) -> None:
    """A background refresh worker cannot race an absent batchd socket during the window."""

    app = gate_live_server.app
    # The scheduler lease is what pins batchd up and keeps its socket from idling out, exactly
    # as the local scheduler does in production before any session-files refresh runs.
...
def test_pinned_batchd_scheduler_lease_is_released_exactly_once_on_teardown(
    ...
    # teardown calls the release from two owners (``stop_background_scheduler`` and
    # ``stop_auto_approve_all``), but ``BatchClient.stop_for_scheduler`` is idempotent and only the
    ...
    stop_fixture_app_runtime(app, label="batchd-scheduler-pin-regression")

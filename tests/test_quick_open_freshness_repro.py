...
1. The web process is a read client for Quick Open.  `search_index_can_build`
   (`yolomux_lib/app.py:2920-2922`) returns False for the search-index role, so only the
   `indexd` child may build.  Read clients take a snapshot-serving branch when a committed
...
3. `refreshing: True` claims some other process is currently refreshing this
   index.  It was emitted with no proof at `filesystem/search.py:497`, `:525`, `:553`,
   `:605` and `:640`, and at `:850` it was literally a reader-role state - a role
   predicate wearing a freshness name.  `:553` looked gated on
   `not refresh_result.get("fallback")`, but `request_background_refresh` returns
...
* `test_bug_reader_serves_stale_snapshot_as_ready_full_while_indexd_is_dead`
  -> `test_fixed_reader_labels_a_stale_snapshot_instead_of_claiming_ready_full`
* `test_bug_refreshing_is_true_with_nothing_refreshing`
  -> `test_fixed_refreshing_is_false_with_nothing_refreshing`
* `test_bug_warming_owner_claims_refreshing_from_a_child_snapshot`
  -> `test_fixed_warming_process_does_not_claim_a_remote_refresh`
* `test_bug_index_status_refreshing_is_only_a_role_predicate`
  -> `test_fixed_index_status_refreshing_is_not_a_role_predicate`
...
def _reader_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, Path, list[dict]]:
    """A root with a persisted snapshot, read by a non-building web process."""
    ...
    _clear_registry()
    # This web process is not the index builder: app.py:2920-2922 returns False for this role.
...
def _reader_root_with_stale_snapshot(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, Path, list[dict]]:
...
def test_fixed_reader_labels_a_stale_snapshot_instead_of_claiming_ready_full(tmp_path, monkeypatch):
    """This is the exact 7771 incident, now labelled instead of hidden.

    A read client with a 200-day-old snapshot on disk and no running
    `indexd` still SERVES the snapshot - refusing to answer is worse - but reports
    `stale` with `unverified` coverage, an explicit reason, the snapshot age and
    the producer state.  It still asks its producer nothing per query.
    """

    root, ghost, indexd_calls = _reader_root_with_stale_snapshot(tmp_path, monkeypatch)
    payload = filesystem.search_files(str(root), query=SENTINEL_QUERY, limit=50, recursive=True)
...
def test_contract_stale_snapshot_must_not_be_served_as_ready_and_full(tmp_path, monkeypatch):
    ...
    `ready` + `full`, and it must carry explicit stale/degraded metadata that the
    Quick Open UI can render inline.
    """

    root, _ghost, _indexd_calls = _reader_root_with_stale_snapshot(tmp_path, monkeypatch)
    payload = filesystem.search_files(str(root), query=SENTINEL_QUERY, limit=50, recursive=True)
    ...
    assert (payload["index_state"], payload["index_coverage"]) != ("ready", "full"), (
...
def test_positive_control_live_producer_snapshot_is_still_served_as_ready_and_full(tmp_path, monkeypatch):
    ...
    root, _ghost, indexd_calls = _reader_root(tmp_path, monkeypatch)
    payload = filesystem.search_files(str(root), query=SENTINEL_QUERY, limit=50, recursive=True)
...
# --------------------------------------------------------------------------------------
# (b) `refreshing` must name an observed refresher.
# --------------------------------------------------------------------------------------


def _refreshing_payload(site: str, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[dict, list[dict]]:
    """Drive one previously-unproven `refreshing` site as a read client with dead indexd."""

    root, _ghost, indexd_calls = _reader_root_with_stale_snapshot(tmp_path, monkeypatch)
...
def test_fixed_refreshing_is_false_with_nothing_refreshing(site, tmp_path, monkeypatch):
    """`refreshing` is a claim about another process, and now needs proof.
    ...
    payload, indexd_calls = _refreshing_payload(site, tmp_path, monkeypatch)
    ...
    assert payload.get("refreshing", False) is False
...
@pytest.mark.parametrize("site", ("search.py:497", "search.py:525", "search.py:640"))
def test_contract_refreshing_requires_a_live_refresher(site, tmp_path, monkeypatch):
    ...
    payload, _indexd_calls = _refreshing_payload(site, tmp_path, monkeypatch)

    assert payload.get("refreshing", False) is False, (
        "refreshing must be derived from an observed live refresher, not from a reader role"
    )


def test_refresh_request_acceptance_controls_warming_state(tmp_path, monkeypatch):
    """A missing requester must not manufacture a warming state; acceptance is the proof."""
    ...
    def payload_for(*, producer_epoch: str, accepting: bool) -> dict:
        ...
        file_index.reset_producer_liveness_cache()
        if accepting:
            file_index.set_background_refresh_requester(lambda _role, _payload: {"ok": True, "accepted": True, "fallback": False})
        else:
    ...
    assert neither["index_state"] == "fallback-skipped"
    assert neither.get("refresh_requested", False) is False

    accepted_only = payload_for(producer_epoch=dead_epoch, accepting=True)
    assert accepted_only["index_state"] == "warming"
    ...
    accepted_live = payload_for(producer_epoch=live_epoch, accepting=True)
    assert accepted_live["index_state"] == "warming"
    assert accepted_live["producer_state"] == file_index.PRODUCER_RUNNING
    assert accepted_live["refresh_requested"] is True


def test_quick_open_does_not_wait_for_cold_index_refresh(tmp_path, monkeypatch):
    """A cold Quick Open refresh is advisory and must not hold the request thread."""
    ...
    root.mkdir()
    file_index.set_build_authority_checker(lambda _role: False)
    file_index.set_background_index_search_requester(
        lambda _payload: {"ok": False, "reason": "cold indexer"}
    )
    ...
    def blocking_enqueue(*_args, **_kwargs) -> dict:
        enqueue_calls.append((_args, _kwargs))
        refresh_entered.set()
        if not release_refresh.wait(timeout=5.0):
            raise AssertionError("cold refresh was not released")
        refresh_finished.set()
        return {"ok": True, "accepted": True, "fallback": False}

    webapp = object.__new__(app_module.TmuxWebtermApp)
    scheduler = app_module.BackgroundScheduler(project_root=str(tmp_path))
    assert scheduler.start() is True
    webapp.background_scheduler = scheduler
    webapp.search_indexer = SimpleNamespace(enqueue=blocking_enqueue)
    webapp.record_performance_sample = lambda *_args, **_kwargs: None
    webapp.mark_search_progress_active = lambda: None
    file_index.set_background_refresh_requester(webapp.request_background_refresh)

    def run_search() -> None:
        try:
            result.append(filesystem.search_files(str(root), query="needle", limit=20, recursive=True))
        except BaseException as error:  # capture the worker failure for the assertion below
            errors.append(error)

    worker = threading.Thread(target=run_search, name="quick-open-cold-refresh-test")
    worker.start()
    try:
        assert refresh_entered.wait(timeout=1.0)
        second = webapp.request_background_refresh(
            app_module.BACKGROUND_ROLE_SEARCH_INDEX,
            {"root": str(root), "query": "needle-2", "reason": "search-index-missing", "advisory": True},
        )
        assert second["deferred_acceptance"] is True
        assert second["coalesced"] is True
        assert scheduler.status_payload()["work_queue"]["pending"] == 1
        worker.join(timeout=0.25)
        assert not worker.is_alive(), "Quick Open waited on cold index refresh work"
    finally:
        release_refresh.set()
        worker.join(timeout=2.0)
        refresh_finished.wait(timeout=2.0)
        scheduler.stop()

    assert not worker.is_alive()
    assert not errors, errors
    assert result and result[0]["index_state"] == "warming"
    assert len(enqueue_calls) == 1


def test_fixed_warming_process_does_not_claim_a_remote_refresh(tmp_path, monkeypatch):
    """search.py:605 - the local process path.

    A build-capable process with no snapshot for `root` serves rows out of an already
    persisted CHILD snapshot.  The only possible refresh would be this process's own
    build, and that build has not started yet.
    """
    ...
    # Freeze the process before its own build thread runs, so no refresh has started
    # while the payload below is produced.
    builds: list[Path] = []
    ...
    assert payload.get("refreshing", False) is False
    assert payload["refresh_requested"] is False
    assert builds == [root], "the local process owns the only scheduled refresh"


def test_indexed_only_reader_serves_available_sqlite_rows_before_warming_fallback(tmp_path, monkeypatch):
    _clear_registry()
    monkeypatch.setattr(file_index, "INDEX_DIR", tmp_path / "idx")
    root = tmp_path / "root"
    root.mkdir()
    ...
    assert [entry["path"] for entry in payload["files"]] == [str(target)]
    assert payload["index_state"] in {"ready", "ready"}
...
def test_contract_warming_process_must_not_claim_a_remote_refresh(tmp_path, monkeypatch):
    """CONTRACT: a process's own warming build is not a remote refresh."""
    ...
    assert payload.get("refreshing", False) is False, "a local warming build is not a remote refresh"
...
def test_fixed_index_status_refreshing_is_not_a_role_predicate(tmp_path, monkeypatch):
    """`search.py:850` used to compute `refreshing` from a reader role.

    `state` still reports the role, because that is a real and separate fact.  The two
    freshness claims beside it - `freshness` and `refreshing` - now come
    ...
    assert status["state"] == "missing", "status reports build state, not a peer role"
    assert status.get("refreshing", False) is False
...
def test_index_status_freshness_survives_for_a_vouched_snapshot(tmp_path, monkeypatch):
    """Positive control for the same projection: a live producer keeps `freshness`."""

    root, _ghost, _indexd_calls = _reader_root(tmp_path, monkeypatch)
    status = filesystem.index_status(str(root))

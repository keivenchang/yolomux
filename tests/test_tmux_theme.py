...
def test_tmux_theme_commands_apply_globals_sessions_and_windows():
    ...
    assert ["set-option", "-g", "status-style", "bg=#2563eb,fg=#ffffff"] in commands
    assert ["set-option", "-t", "1:", "status-style", "bg=#2563eb,fg=#ffffff"] in commands
    assert ["set-window-option", "-t", "8003:2", "window-status-current-style", "bg=#3b82f6,fg=#ffffff,bold"] in commands
    assert ["set-window-option", "-t", "1:0", "pane-active-border-style", "fg=#2563eb"] in commands
...
def test_apply_tmux_theme_color_to_existing_lists_then_updates_every_target():
    ...
    assert (["set-option", "-t", "2:", "status-right-style", "bg=#2563eb,fg=#ffffff"], 3.0) in calls
    assert (["set-window-option", "-t", "2:1", "pane-active-border-style", "fg=#2563eb"], 3.0) in calls

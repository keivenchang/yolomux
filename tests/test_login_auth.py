...
def test_test_auth_bypass_env_allows_api_verification_without_cookie(monkeypatch, tmp_path):
    ...
    app = SimpleNamespace(
        sessions=[],
        dangerously_yolo=False,
        settings_payload=lambda: {"settings": {"appearance": {"theme": "light"}}},
    )
    server, thread = start_server(monkeypatch, tmp_path, app=app)
    port = server.server_address[1]
    try:
        ...
        assert status == HTTPStatus.OK
        assert json.loads(body)["settings"]["appearance"]["theme"] == "light"
...
def test_pricing_catalog_routes_are_readonly_visible_and_admin_refresh(monkeypatch, tmp_path):
    refreshes = []
    app = SimpleNamespace(
        sessions=[],
        dangerously_yolo=False,

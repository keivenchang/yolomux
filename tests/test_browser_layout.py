...
def test_current_stats_system_tab_order_visible_polling_refresh_scroll_and_narrow_layout(browser, tmp_path):
    ...
    assert metrics == {
        ...
        "resolution": True,
        "toggles": 11,
        "toggleLabels": ["CPU", "Daemons load", "System memory", "Agents", "Session tokens/min", "Model output tokens/min", "GPU utilization", "GPU memory", "Client latency", "Client API&SSE/sec", "Client bandwidth/sec"],
...
def test_current_stats_cpu_stacks_host_binaries_and_the_daemon_chart_emphasizes_the_yolomux_line(browser, tmp_path):
    load_live_runtime_boot_fixture(browser, tmp_path, "?debug=1&sessions=debug")
    WebDriverWait(browser, 8).until(lambda driver: driver.execute_script("return document.querySelector('.js-debug-panel [data-js-debug-graph]') !== null"))
    result = browser.execute_script(
        """
            ...
            // The yolomux.py web line renders with the daemons now, not beside the host binaries.
            const daemonCard = graph.querySelector('[data-js-debug-chart="serversLoad"]');
            ...
            const yolomuxLine = daemonCard?.querySelector('[data-js-debug-series^="cpu:port:"]');
            const yolomuxOnCpuCard = card?.querySelector('[data-js-debug-series^="cpu:port:"]');
            const systemLegend = card?.querySelector('[data-js-debug-legend="systemCpu"] .js-debug-legend-line .js-debug-line');
            const yolomuxLegend = daemonCard?.querySelector('[data-js-debug-legend^="cpu:port:"] .js-debug-legend-line .js-debug-line');
            const legendOrder = [...(daemonCard?.querySelectorAll('[data-js-debug-legend]') || [])].map(node => node.dataset.jsDebugLegend);
            const linePaintOrder = [...(daemonCard?.querySelectorAll('.js-debug-line-chart [data-js-debug-series]') || [])].map(node => node.dataset.jsDebugSeries);
            const cpuCardLegendOrder = [...(card?.querySelectorAll('[data-js-debug-legend]') || [])].map(node => node.dataset.jsDebugLegend);
            ...
            return {
              ...
              yolomuxOnCpuCard: Boolean(yolomuxOnCpuCard), cpuCardLegendOrder,
    ...
    # The serving-port line keeps its guarantee -- first in the legend, painted last, emphasized,
    # solid -- but it keeps it on the daemon chart, where the web process belongs with its peers.
    ...
    # And it is not drawn twice: the CPU card keeps the host binaries and the system total only.
    assert result["yolomuxOnCpuCard"] is False, result
    assert not any(key.startswith("cpu:port:") for key in result["cpuCardLegendOrder"]), result
...
def test_yocost_preferences_and_retained_totals_show_marginal_and_api_list_prices(browser, tmp_path):
            ...
            provider: 'openai', model: 'gpt-subscription', label: 'gpt-subscription',
             ...
             dimensions, priced: {atoms: 1, tokens: 1000}, unpriced: {atoms: 0, tokens: 0},
              ...
              const anthropicDimensions = {
                ...dimensions,
                cache_write_5m: {tokens: 100, micro_usd: 750000, api_list_micro_usd: 750000},
                cache_write_1h: {tokens: 200, micro_usd: 4000000, api_list_micro_usd: 4000000},
              };
           const anthropicRow = {
            ...pricedRow,
          ...
          return {
            ...
            legend: report ? {
              ...
              agentHeading: report.querySelector('.js-debug-cost-agent-usages h2')?.textContent.trim() || '',
               agentName: (() => {
                 const node = renderedReport.querySelector('[data-js-debug-cost-table="agent"] tbody .js-debug-cost-agent-name') || renderedReport.querySelector('[data-js-debug-cost-table="agent"] tbody strong');
                return node ? {
              ...
              pricingTitle: renderedReport.querySelector('[data-js-debug-cost-table="model"] .js-debug-cost-pricing-links--compact a')?.title || '',
               pricingBefore: (() => { const node = renderedReport.querySelector('[data-js-debug-cost-table="model"] .js-debug-cost-pricing-links--compact'); return node ? getComputedStyle(node, '::before').content : ''; })(),
               pricingAfter: (() => { const node = renderedReport.querySelector('[data-js-debug-cost-table="model"] .js-debug-cost-pricing-links--compact'); return node ? getComputedStyle(node, '::after').content : ''; })(),
    ...
    assert len(result["legend"]["entries"]) == 6, result
    assert any(entry.startswith("Cache") and "hits and refreshes" in entry for entry in result["legend"]["entries"]), result
    assert any(entry.startswith("Cache write") and "cache creation" in entry for entry in result["legend"]["entries"]), result
    assert set(result["legend"]["swatches"]) == {
        "js-debug-cost-usage-swatch js-debug-cost-usage-swatch--input",
        "js-debug-cost-usage-swatch js-debug-cost-usage-swatch--cache_read",
        "js-debug-cost-usage-swatch js-debug-cost-usage-swatch--cache_write",
    ...
    assert result["legend"]["anthropicCells"][2].startswith("100 · $0.7500"), result
    assert result["legend"]["anthropicCells"][3].startswith("200 · $4.00"), result
    assert result["legend"]["modelFormulae"] == ["1.0k x $6.00/1M = $0.00"], result
    assert result["legend"]["pricingText"] == "", result
    assert result["legend"]["pricingTitle"] == "", result

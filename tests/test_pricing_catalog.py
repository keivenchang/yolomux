...
def test_refresh_uses_etag_and_activates_normalized_official_catalog(tmp_path):
    ...
    assert result == {"ok": True, "status": "updated", "catalog_revision": 2}
    rate = catalog.resolve_rate(provider="openai", model="gpt-4.1", direction="input")
...
def test_refresh_removed_official_model_does_not_resolve_from_historical_revision(tmp_path):
    now = [100.0]
    catalog = PricingCatalog(tmp_path, clock=lambda: now[0])
    ...
    now[0] = 116.0
    assert catalog.refresh([adapter], fetch=lambda *_args: (200, {}, json.dumps(payload("gpt-replacement")).encode()))["status"] == "updated"
    assert catalog.resolve_rate(provider="openai", model="gpt-removed", direction="input") is None
...
def test_corroboration_json_parsers_never_claim_official_status():
...
def test_public_rate_and_catalog_payload_expose_only_safe_source_evidence(tmp_path):
    ...
    assert evidence["source_url"] == "https://platform.openai.com/docs/pricing"
    public = catalog.public_payload()
    assert public["status"]["state"] == "seed-only"
    assert public["sources"] == [{"kind": "seed", "url": "https://platform.openai.com/docs/pricing", "revision": 1}]

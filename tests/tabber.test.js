...
async function runTabberSuite() {
  ...
  test('active drags defer panel rendering and preserve tab chrome', () => {
    ...
    assert.match(layoutState, /function completeLayoutMutationGeneration[\s\S]*?!runtimeState\.completeLayoutMutation\(generation\)/, 'the shared scheduler publishes completed layout generations');

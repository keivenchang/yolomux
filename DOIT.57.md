...
- Upload destination — defaulting uploads into a `.uploads/` subdir (Preferences `uploads.subdir`) is tracked separately in **DOIT.59**. Complementary: once it ships, this feature's external-file drops will land in `<cwd>/.uploads/` automatically.

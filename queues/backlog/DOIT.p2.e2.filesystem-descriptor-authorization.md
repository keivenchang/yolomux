...
- [x] Route create, write, rename, and delete through an authorized parent descriptor plus exactly one validated basename. Preserve configured-root refusal, expected-mtime conflict behavior, Git-aware moves, atomic replacement, and path-policy cache invalidation without re-resolving an attacker-controlled parent. DONE (landed earlier): mutations route through `safe_parent` plus one validated basename; configured-root refusal is preserved via `_ensure_not_configured_root`; rename/delete preserve behavior with the final basename syscall as the approved linearization point and the residual race documented above.

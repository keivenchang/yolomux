...
VERIFIED NOT A HOLE: `delete` is absent from `FILESYSTEM_RETAINED_READ_OPERATIONS`, so it always gets a fresh `uuid` coalesce key, and batchd's inline-ready branch only fires on an already-stored product. A `pending` payload can therefore never escape through the synchronous HTTP return, so the escalation owner is genuinely singular.
...
- [ ] Route bounded unlink through the mutation lane and recursive deletion through a bulk receipt, preserving symlink safety, exact target authority, conflict behavior, Git/index invalidation, failure normalization, and UI state. REOPENED: the lane split and same-receipt escalation exist, but the current escalation re-produces a raw pathname after the original descriptor probe. Namespace replacement can therefore change the recursive target before the bulk owner reauthorizes it, so exact target authority and the downstream invalidation claims remain unproved.

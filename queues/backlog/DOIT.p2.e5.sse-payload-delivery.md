# DOIT.p2.e5.sse-payload-delivery.md - Reduce Push Payloads And Prove Graph Delivery

Source provenance: `DOIT.p2.md` P2-D and the former `DOIT.p2.e5.sse-payload-delivery.md`.
...
This lane owns push payload shape, terminal cursor/repair semantics, and graph delivery under load. It consumes phase metrics from `DOIT.p2.e4.latency-boundaries.md` and must not absorb generic page-load coordination.
...
- Phase metrics from `DOIT.p2.e4.latency-boundaries.md` improve attribution but do not block deterministic byte/cursor tests. Generic refresh deduplication remains in `DOIT.p1.e5.refresh-fanout-background-cpu.md`.

...
- [ ] Calm the "working" YO badge — it FLASHES distractingly. `.session-yolo-marker.active.working` (`yolomux.css` ~855) runs `yolo-ring-fade` + `yolo-pill-fill-fade` with `animation-iteration-count: infinite` at `--yolo-reminder-duration` (≈ `red_reminder_ms` ≈ 1.5s) — i.e. the RED attention pulse — for a session that is merely *working* (not needs-attention). RECOMMENDATION: (1) for `working`, keep ONLY `yolo-marker-rotate` (the slow 20s spin) as a calm "in progress" cue and DROP the red ring/pill fades; (2) reserve the red ring/pill pulse for genuine needs-attention states (needs-input / needs-approval / blocked) so red still means "look at me" (don't cry wolf); (3) phase-anchor any kept fade with a negative `animation-delay` (like the rotate already uses `--yolo-rotate-delay`) so a re-render that moves the marker node can't restart it mid-cycle. Net: working = quiet spin, no flashing; red flash only when the session needs you. NOTE: it's a CSS animation, not the data refresh — the reconcile/poll are fine.

- [ ] Tab hover popover FLICKERS — shows, disappears momentarily, then comes back. Root cause (researched 2026-05-30): the popover is a CHILD of the tab node (`.pane-tab > .session-popover`, shown via `.pane-tab.popover-open > .session-popover`, `yolomux.css` ~2316), and `renderPaneTabStrips` rebuilds the whole strip with `strip.replaceChildren(...createPaneTab)` (`yolomux.js` ~8027) — destroying the hovered tab + its popover. The restore logic (`paneTabPopoverItemToRestore` -> rebuild -> `restorePaneTabPopover`, ~8024-8048) re-creates the popover on the new node a moment later, so it blinks out and back. Triggered by ANY routine re-render while hovering: the ~15s metadata poll (`refreshTranscripts` -> `renderSessionButtons` + `renderPaneTabStrips`) plus the many other render call sites (auto-approve state, settings-apply, ws events). FIX: make `renderPaneTabStrips` RECONCILE tab nodes by key (reuse the hovered tab + its popover, update only changed attrs) instead of `replaceChildren` — same pattern the finder already uses (`renderTreeChildren` reconcile); OR skip/guard the strip rebuild while a popover is open or when nothing changed. Same family as the finder-flash and Preferences-popup re-render bugs.
...
- [ ] YOLO approver doesn't reliably auto-approve the NEW Claude prompt format. The current Claude Code permission prompt — a `Bash command (unsandboxed)` box + `Permission rule Bash requires confirmation` + `Do you want to proceed? 1. Yes 2. No` + the new footer `Esc to cancel · Tab to amend · ctrl+e to explain` — sits unapproved (observed live 2026-05-30). VERIFY `approval_prompt_state` in `auto_approve_tmux.py` against this layout, especially: (a) `yes_selected` detection — if the new UI doesn't pre-highlight "Yes" the way the detector expects, the worker hits "prompt found, Yes not selected" and never presses; (b) the new footer line not throwing off the prompt-box boundary or `extract_command`. Capture a real pane via `tmux capture-pane -p` and add it as a fixture to the detector's test table (the `check(...)` cases, ~1081). NOTE: not blocked by the default-policy fix (that was a separate, earlier issue, now fixed).

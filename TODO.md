...
- [ ] Find / tab quick-switcher: a search box where the user types a keyword and it lists the matching Tab(s) to jump to. Search the rich per-session metadata — PR number (`pull_request.number`), Linear IDs (`extract_linear_ids` / `linear_ids`, grep in `metadata.py`), branch, commit-style title/subject, session name/number. This is the full version of the `Tab` menu's `type-to-filter` (Chrome-style "Search Tabs"): match those fields, show matching rows, Enter jumps/activates the tab. Surface it at the top of the `Tab` menu and/or as a global command-palette hotkey.
...
- [ ] Background summarizer CLI + inferred working path -> restore the Tab->jump-to-Finder action. A CLI / background loop that periodically walks each session and generates a rolling summary, reusing the existing AI-summary plumbing (`codex_summary_prompt` in `yolomux_lib/transcripts.py` ~134 + `app.py` ~429; `/api/summary-stream`; `SUMMARY_CODEX_MODEL`/EFFORT/SERVICE_TIER in `common.py`). Beyond the summary text, INFER the agent's effective working directory from the transcript (the files claude/codex is actually editing / `cd`s into) — more accurate than the raw process cwd. Use that inferred path to re-implement the per-tab "jump the File Explorer to this session's working path" action that was removed earlier, and to upgrade the root-mode "sync to tmux session" (DOIT.5 #2) to use the inferred path instead of process cwd. Run on an interval, throttled, and only when a session is quiet (per the P0 "delay LLM classification until quiet" note); persist the latest summary + inferred path per session. Cost-gate behind a setting / interval (Preferences).
...
- [ ] RESEARCH: does the PR pill (`#<num>`) show at the same time as the CI pill, and is one redundant? The tab badges are separate spans (grep `pr-indicator` / `ci-indicator` in `static/yolomux.js`): a `#<PR>` pill (`pr-indicator`, ~5768), a `CI` pill (~5775), a `status` pill (~5763), and `MAIN` (~4766), all colored by `pullRequestStatusClass`. Investigate when each appears — specifically whether the `#PR` pill AND the `CI` pill both render when CI is green, and whether that's redundant (the PR pill already carries status color; a separate green CI box may add nothing). Propose consolidating: e.g. show `#PR` colored by CI state and drop the separate `CI` box when it's green, or only show `CI` when it's NOT green. Don't change behavior until the research + proposal is reviewed.

...
- Current LIVE top-level menus: `File`, `View`, `Panes`, `YOLO`, `Settings`, `Help`. Gaps vs the design tree below:
  - NAMING DECISION (2026-05-29): rename the `Panes` menu to `Tab` (singular, Chrome-style). See the "Rename Panes -> Tab" TODO below. This supersedes the earlier "use Panes" and the brief "Windows" idea — `Tab` is accurate because the menu lists/navigates Tabs, and it avoids overloading `window` (which already means a tmux window via the `<`/`>` step).
...
- [x] Reconcile the live menu (`File/View/Panes/YOLO/Settings/Help`) with the design tree below — `Log out` stays in File for now and can be mirrored in Settings later. (Naming: the `Panes` label is being renamed to `Tab` — see the rename TODO below.)
- [ ] Rename `Panes` menu -> `Tab` (singular, like Chrome's `Tab` menu). Scope: the `appMenuTree()` entry `id: 'panes'` / `label: 'Panes'` (grep `id: 'panes'`); the top-bar layout and design tree in this section (the `Tab ▾` heading and references); and the README terminology section (`README.md` line ~212 describes the "`Panes` menu" with Active/Background/Inactive grouping — rename to "`Tab` menu"). This name matches the glossary (the menu lists Tabs), so the README's pane/tab/window definitions stay correct — only the menu label changes; do NOT rename the `pane` (split region) or `window` (tmux window) concepts.
...
Menu naming follows the latest YOLOmux terminology: `File` (create/manage), `View` (display options), `Tab` (Chrome-style — lists and navigates open tabs: active, background, inactive), plus app-specific `YOLO` and `Settings`, and `Help`.
...
- [ ] Tab ▾  (navigate — a FLAT list of active, background, and inactive tabs, with checkmark/highlight on the active one, click/Enter to focus, type-to-filter)
...
- [ ] Multi-line tab wrapping: when a pane's tabs wrap to 2+ rows (the `fix: wrap crowded tabs` work, commit `e5df095`), the second row and beyond should use the full width INCLUDING the area beneath the pane toolbar (`<` / terminal / `Tx` / `AI` / `Log` / `Info` / `×`), instead of stopping short and leaving that space blank. Only the FIRST row needs to reserve room for the toolbar. Where: `.panel-head` contains `.window-session-tabs` (the wrapping session tabs) plus the toolbar from `panelControlsHtml` (the `class="tabs"` strip). Idea: make the toolbar `position: absolute; top:0; right:0` inside the `position: relative` `.panel-head` (or float it right) and let `.window-session-tabs` span the full width so wrapped rows flow under the toolbar; only the first row clears it. A floated toolbar with inline-block tabs gives this wrap-around for free (flex items do not wrap around floats, so this likely needs the tab items to be inline-block rather than flex). Verify against the existing `flex-wrap` rule (grep `flex-wrap` in `static/yolomux.css`).
- [x] Keep an EMPTY placeholder pane (don't collapse the split) when a split pane's last tab closes, to preserve the sibling pane's size. Concrete case: left = Finder (File Explorer), right = a tab. When the right side's last tab is closed/killed, do NOT collapse the split — collapsing lets Finder reflow to full width and loses its sizing. Instead keep an empty placeholder pane on the right so Finder stays right-sized, and the user can drop a new tab into the empty slot. The codebase already has the placeholder concept: `emptyPlaceholderPaneState()` -> `{tabs: [], placeholder: true}` (grep `placeholder` / `emptyPlaceholderPaneState`); placeholder panes are kept, but `compactLayoutNode` / `paneHasLayoutContent` PRUNE a pane that empties without being marked placeholder. Fix: on close/kill of a pane's last tab, mark that pane as a placeholder instead of pruning it. Only collapse the split fully when ALL panes are empty, or when the user explicitly closes the pane itself (not just its tab). This is the concrete "empty state" referenced by the kill-removal item under "Session roster lag".
...
- [ ] Fix awkward hover timing (classic menubar behavior). FIRST open (no menu currently open) should wait a few hundred ms before popping on hover — but the current delay `menuHoverOpenDelayMs` is 800 ms (grep `menuHoverOpenDelayMs`), too long; drop it to ~200-300 ms. Once a menu is ALREADY open, moving the pointer to another top-level menu (or into its submenus) must switch INSTANTLY with no delay. Fix in `queueAppMenuHoverOpen` (grep the function): if `appMenuIsOpen()` is already true, open the hovered menu immediately (skip the timer); otherwise use the short first-open delay.

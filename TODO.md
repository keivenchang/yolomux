...
- Current LIVE top-level menus: `File`, `View`, `tmux`, `Windows`, `Help`. Gaps vs the design tree below:
  - NAMING DECISION (2026-05-30): the menu that lists the visible/minimized/inactive app tabs is named `Windows`, while `tmux` is only for tmux-specific controls. The dropdown rows are separated by bars only; no `Active` / `Minimized` / `Inactive` section text.
...
- [x] Reconcile the live menu (`File/View/tmux/Windows/Help`) with the design tree below — `Log out` stays in File and the top-right controls.
- [x] Rename the app-tab list menu to `Windows`. `tmux` is now tmux-scoped actions only; `Windows` lists the app's open panes/tabs (tmux sessions, Finder, editors, viewers, Preferences, Branch Info) with rich rows.
...
- Safari: a `Show All Tabs` overview (grid of all tabs) — a richer alternative/companion to the dropdown tab list; and it splits tab actions between `File` (New Tab) and `Window` (Next/Previous, Move Tab). We consolidate navigation into the `Windows` menu instead — cleaner.
- iTerm separation, which our top-level split mirrors: `Shell` = create (New Window/Tab, Split, Close) -> our `File`; `Session` = per-session actions (Edit/Rename, Restart, Logging) -> our per-pane kebab + right-click-tab menu; `Window` = navigate (Select Next/Previous Tab, Select Next/Previous Pane, Move Tab, list) -> our `Windows` menu + keyboard nav. Also borrow iTerm `Edit Tab Title` (= our rename) and `View -> Toggle Tab Bar` (= our tab-metadata / panel-tabs toggles).
- Net structure to converge on: `File` = create/open (Chrome File + iTerm Shell), `Windows` = navigate/act-on-tabs (Chrome Tab + iTerm Window's tab/pane selection), `View` = display toggles (all three), per-pane kebab + right-click = per-session actions (Chrome right-click-tab + iTerm Session).

Proposed top-bar layout (left to right): `YOLOmux ver` · `[File ▾] [View ▾] [tmux ▾] [Windows ▾] [Help ▾]` · active-session chip (label + state badge) · latency · `Notify` · `Refresh` · status · `Log out`.
...
- [ ] View ▾  (display options)
  ...
  - Branch Info: show — opens the Branch Info viewer for the active session (also still available as the per-pane `Info` tab; once open it appears in the Windows menu)
- [ ] Windows ▾  (navigate — a flat rich-row list of active, minimized, and inactive tabs, with checkmark/highlight on the active one, click/Enter to focus, type-to-filter)
...
- [ ] Per-pane left dropdown (the caret on the LEFT of each panel's tab strip): looks IDENTICAL to the Windows ▾ menu above (same compact rich-row format), but scoped to just THAT pane's tabs — i.e. only the tabs/views belonging to that one panel (its tmux sessions + Terminal / Tx / AI / Log / Info), not every pane in the app. Same row styling, same active-row highlight.
...
- [ ] App-level keyboard shortcuts: Ctrl/Cmd+B toggles the File Explorer / Finder on/off; Ctrl/Cmd+`,` (comma) opens the Preferences tab. Add a document-level `keydown` handler (there is no global shortcut registry today — the only Ctrl/Cmd+S handler is bound to `#fileEditorTextarea`, grep `event.key === 's'`). Cmd+B -> toggle: if `itemInLayout(fileExplorerItemId)` close it, else `selectSession(fileExplorerItemId)` (grep both). Cmd+`,` -> open the Preferences tab (`prefsItemId`). CONFLICT TO HANDLE: Ctrl-B is the tmux PREFIX key — when a terminal pane is focused, Ctrl-B MUST pass through to tmux, so only fire the YOLOmux shortcut when focus is NOT inside a terminal (or make it Cmd-only on mac / a configurable binding). Also register both in the Help -> Keyboard shortcuts submenu so they're discoverable.
...
- [ ] Implement the `View ▸ Layout` commands `Single pane` and `Split` (today `Single pane` / `Grid` / `Wall` are disabled "Coming soon" placeholders, `appMenuTree` ~2837; there is no `Split` item yet — add it). Both rearrange ONLY the tabs CURRENTLY assigned to panes (visible), never pulling in hidden ones (minimized / inactive), and NEITHER touches Finder — leave the File Explorer pane exactly where/whether it is.
  - **Single pane:** collect every tab currently in a pane across all active slots EXCEPT Finder (use `paneTabsWithoutFinder(slot, slots)`, ~6113, over `layoutSlotKeys()`), and consolidate them — in their current order, de-duped — into ONE pane; apply via `applyLayoutSlots(...)` (~2426). Do not add minimized/inactive tabs. If Finder occupies a pane, it keeps its own pane (the other tabs collapse into the single pane beside it); if Finder isn't present, the result is genuinely one pane.
  - **Split:** arrange the same set (current pane tabs, minus Finder, no hidden ones) across left/right (`basePaneKeys = ['left','right']`) — best-effort: keep each tab on the side it's already on when it has one, otherwise split by order roughly in half. Again leave Finder's pane untouched. Apply via `applyLayoutSlots(...)`.
  - Validate: with 3 visible tabs spread across panes + 2 minimized, `Single pane` shows all 3 in one pane (not the 2 hidden), `Split` shows them across left/right; in both, an open Finder stays put.
...
- [ ] Add a changed-files list and diff panel using the session cwd. Scoped read-only today, but if the CodeMirror path wins (see `EDITOR-CODEMIRROR.md`), `@codemirror/merge` makes this an EDITABLE diff (unified inline + side-by-side, accept/reject + revert chunks, edit while diffing) instead of read-only.
...
- [ ] In-document Find (`Cmd/Ctrl+F`). DISTINCT from the global command palette (`Cmd/Ctrl+K`, which searches tabs/menu/settings) — this searches WITHIN the open file. Intercept `Cmd/Ctrl+F` in the editor textarea keydown (today only `Cmd/Ctrl+S` is caught, ~4990) so the browser's native find doesn't fire over the transparent-overlay editor. Show a find bar: query input + match count `N/total` (e.g. `1/13`) + prev `^` / next `v` + close `×`. Typing finds matches in the textarea value; jump to + REVEAL the current match via `setSelectionRange` + scroll (sync the syntax overlay scroll). `Enter` / `v` = next, `Shift+Enter` / `^` = prev, `Esc` = close; wrap around at ends. Options: case-sensitive, whole-word, regex. Highlight ALL matches by drawing rectangles in the overlay layer at match positions (a `<textarea>` can't style ranges) — at minimum select + reveal the current match. Works in Edit and Split modes.
...
- [ ] DECISION/SPIKE: CodeMirror 6 (third-party license/free) vs extend the textarea — full 2-path plan, success gate, and the concrete "remove the textarea editor layer" list in `EDITOR-CODEMIRROR.md`. CM would close #6 + most of the editor-options list above. Promote to a DOIT once the direction is approved.
...
- [ ] The pane `+` (expand) button is too BRIGHT. It's `pane-expand` (`yolomux.js` ~9849) styled with the `pc-zoom` class, whose background is `#52d273` (the `--good` bright green, CSS ~64). Tone it down so it doesn't dominate the toolbar — e.g. a muted/desaturated fill (or neutral like the other controls) at rest and only brighten on hover/focus, matching the weight of the `_` minimize and `...` actions buttons next to it.

- [x] Clicking a YO badge in the `Windows` dropdown row toggles YOLO without re-sorting or closing the menu.
- [x] Dense `Windows` dropdown rows stay readable on small viewports; old tmux/YOLO session-submenu rows were removed when sessions moved under `Windows`.
...
- [ ] Remove the redundant `CI` pill (user: no redundant info). RESEARCHED 2026-05-29: the tab composes `[#PR, status, CI]` in `pullRequestCompactBadgesHtml`. `pullRequestPrIndicatorHtml` renders `#<num>` whenever a PR exists, COLORED BY `pullRequestStatusClass` (failing/pending/passing/merged/…). `pullRequestCiIndicatorHtml` renders a literal `CI` box whenever checks exist and the PR is not merged, ALSO colored by the SAME `pullRequestStatusClass`. So the CI box's color is always identical to the `#PR` pill's color — when CI is green you get `#1234`(green) + `CI`(green): two boxes, same info. The CI box adds no color beyond the `#PR` pill; only the word "CI". FIX: in `pullRequestCiIndicatorHtml` (grep), return '' when `pr.checks.state` is success/passing — keep the box only for pending/failing (where it flags attention), OR drop the CI box entirely and rely on the `#PR` pill color. Validate: a passing-CI open PR shows just `#1234`(green) with no separate CI box; pending/failing still surfaces. RE-VERIFIED 2026-05-30 AND BROADER: the row actually shows THREE things — a green `PR` (the session-STATE badge `ready-review`, short text `PR`, set BECAUSE CI is passing, `sessionState` ~1738) + a dark `#<num>` pill + a green `CI` box. So "CI passing" is encoded by BOTH the green `PR` state badge AND the green `CI` box — that is exactly the redundancy the user flagged. When CI passes, the `ready-review`/`PR` badge already says "ready", so the separate green `CI` box is the redundant one; keep `CI` only for pending/failing.
- [ ] Double-click a tab to RENAME it (no need to "open" an already-open tab). Double-clicking a pane tab re-activates a tab that's already shown — a no-op. Repurpose double-click to rename: for a tmux-session tab -> `renameTmuxSession` / `beginPaneTabRename` (grep); for a file / image / editor tab -> rename the underlying file (reuse the file-rename affordance, grep `beginFileTreeRename`). Bind `dblclick` on the pane tab (grep the tab's existing `dblclick` handler) to the appropriate rename. Single-click still activates/focuses.
...
- [x] In the `Windows` dropdown rows, the YO marker is clickable to toggle YOLO WITHOUT closing the menu. Rows are built by `menuTabCommand` and render `yoloMarkerHtml(..., {toggle: true})`; the click handler intercepts `[data-auto-session]`, prevents row activation, keeps the dropdown open, and re-renders the marker state.

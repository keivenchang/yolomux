...
Archive status: valid phase-attribution work moved to `DOIT.p2.e4.latency-boundaries.md`; refresh, watch-root, Finder-batch, deepcopy, jobd, statsd, and statusd work moved to `DOIT.p1.e5.refresh-fanout-background-cpu.md`; activity summary remained in its P0-disable/P1-replacement chain; SSE deltas moved to `DOIT.p2.e5.sse-payload-delivery.md`; copy feedback moved to `DOIT.p2.copy-feedback.md`. Those later queues were abandoned on 2026-08-23 without calling their requirements complete. The rejected security/performance shortcuts remain below as historical guardrails, not active checkboxes.
...
Owners recorded at archive time: native network-filesystem watch admission (`DOIT.p1.e4.native-watch-network-fs.md`), refresh fan-out and background CPU (`DOIT.p1.e5.refresh-fanout-background-cpu.md`), latency phase attribution (`DOIT.p2.e4.latency-boundaries.md`), and SSE payload delivery (`DOIT.p2.e5.sse-payload-delivery.md`). Those queues were later abandoned. Copy feedback and terminal wheel consistency were verified complete and archived by the [v0.7.7 queue reconciliation](DONE.0-7-7-active-queue-reconciliation.md).

...
- Tabber shows a live tree of everything open and defaults to fully expanded. Level 0 = tmux sessions (sorted first), then the other open tabs (Preferences, YO!info/YO!agent, file editors) as leaf rows at the end. The session row's name column carries the session number plus its work description (the PR/branch summary) and takes priority — when the pane is narrow the trailing date yields so the name is not truncated to make room. Level 1 = the session's tmux windows (`index:process`, e.g. `0:claude`, tinted by agent); the session's current tmux window is marked with a trailing `●`. Under a `claude`/`codex` window, Level 2 = the paths that agent touched grouped by repo (from `/api/session-files`, lazily fetched + cached per session), with the git branch on the repo row, and Level 3 = the individual files. Non-agent windows (e.g. `bash`) are leaf rows.
- Rows render through the SHARED Finder/Differ row pipeline (`renderTreeChildren` -> `updateFileTreeRow` -> `updateFileTreeRowContents`) via a `mode: 'tabber'` render option whose per-row display values are precomputed as data — never a bespoke per-pane row builder. Selection, the icon column, and the shared date modes (`None` / `Date` / `Ago`) all apply unchanged. The collapse state is a persisted COLLAPSED set (default expanded) keyed by a stable id-based node path (`s_<id>/w_<i>/r_<n>/f_<n>`), so it survives reloads and a session add/remove never shifts another node's open/closed state. Clicking a row's disclosure icon toggles that node; the toolbar `Expand all` / `Collapse all` apply to the whole Tabber tree.
- Time column + sort: each row's time is the activity-ledger recency (`max(last_user_input_ts, last_agent_active_ts, last_output_ts)`); a parent row inherits the most-recent time among its children, and a window with no ledger/child time falls back to the session's recency so every window shows a time. The tree honors the active Finder sort control — `A-Z`/`Z-A` sort by the human label, `new`/`old` by the recency time — and re-sorts on the normal poll without collapsing the tree (collapse state is keyed by the stable path, not display order). The `/api/activity` recency is polled only while the Tabber is the active mode.
- Row actions: a session or other-tab row click opens/activates that tab; a window row click opens the tab and switches to that tmux window (shared `tmuxWindow({windowIndex})` path); a repo or touched-path row click points the Finder at that path (switching to Finder mode) and also opens the tab + switches the window; right-clicking a path row reuses the shared file context menu (targeting the file's real `abs_path`). Hover/passive focus never act.
...
- Dragging a pinned tab to another pane must insert it as the first tab in the target pane. A pinned tab must not be draggable behind non-pinned tabs, even during cross-pane moves or live Dockview previews.

...
- Tabber shows a live tree of everything open. Level 0 = tabs with tmux sessions sorted first (file tabs, Preferences, and YO!info are leaf rows, added later). Level 1 = the session's tmux windows (`index:process`, e.g. `0:claude`, tinted by agent). Level 2 = each window's panes (foreground process plus the basename of its cwd). Level 3 = the context paths each agent touched (from `/api/session-files`, lazily fetched and cached when the session is expanded), grouped by repo with the git branch on the repo row.

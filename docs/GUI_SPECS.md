...
- File or directory drops into terminal content insert the path first. In agent panes, a transient non-modal suggestion overlay may then offer context-aware actions on `Alt+1` through `Alt+9`, such as appending `; do OCR on this image`; external/upload drops use the same overlay after the upload path is known. Escape, click-away, or timeout dismisses it.

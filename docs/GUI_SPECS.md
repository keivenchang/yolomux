...
- Differ file rows open the diff on a normal single left-click in the active non-Finder editor pane; modifier clicks remain selection-only for multi-select. Normal row opens reuse the Differ preview editor tab and start that editor item with collapsed unchanged context without changing the global/default diff expansion setting. Transcript-touched `T` rows still request the Diff editor first when the file is tracked and has useful git history; only a proven unavailable/non-renderable diff falls back to Edit, and that fallback must show a `diff unavailable: ...` status. The row context menu must expose `Open in a new Diff Editor` followed by `Open in a new Editor`; those explicit actions create distinct editor tabs for the same backing file.
...
- Many Dockview tabs must stay in one row. They may narrow to the compact tab width and then horizontally overflow with hidden scrollbars, but they must not wrap underneath the active tab row or overlap the editor toolbar/details row.
...
- Background topbar refreshes must not detach a focused native topbar control. If the language picker or a future topbar input/select is active, passive topbar renders defer until the control blurs; explicit user-triggered forced renders may still rebuild immediately.
...
- `tests/test_browser_layout.py::test_dockview_file_editor_tabs_stay_above_toolbar` covers the editor-specific tab/header contract: many file tabs remain one row, stay above the toolbar, and the active tab remains clickable.

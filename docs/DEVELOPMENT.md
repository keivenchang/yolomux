...
YO!stats retains 24 hours. The newest hour uses one-second buckets, ages one-to-two hours use ten-second buckets, and ages two-to-24 hours use sixty-second buckets, so a fully populated history family has at most 3,600 raw time slots plus 360 middle-tier slots plus 1,320 old-tier slots (5,280 total). `STATE_DIR/tmux-AI-status.json` stores the elected owner's shared series, while `STATE_DIR/stats-client-history.json` stores per-browser and per-server records; disk and heap use therefore scale with the number of populated client/process records inside those bounded time slots, not only with elapsed time. Every YOLOmux server that serves stats expands the shared snapshots into its own in-memory bucket maps, and a browser retains the ranges it has fetched until the 24-hour client compactor removes them. Use file sizes plus bucket/client/server counts for storage audits, and measure the expanded maps rather than attributing the process's full RSS to YO!stats.

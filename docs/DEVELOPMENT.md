...
The static build is also the CSS/source-ownership gate. It rejects duplicate properties inside one rule, duplicate normalized selectors inside the same at-rule context, empty style rules, undefined custom properties, raw colors that duplicate an existing semantic token even when one spelling is hex and another is opaque `rgb()`, and repeated component literals that should have one token owner. Fix the shared token/base selector or move equivalent components onto one shared class; do not add a local exception or another parallel shade.

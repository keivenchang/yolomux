...
`python3 tools/check.py` runs the local gate as parallel lanes and exits non-zero if any lane fails — use it as the one pre-commit/CPS command. Default/full runs and focused expensive lanes serialize through `/tmp/yolomux-expensive-tools.lock`, and when live YOLOmux servers are present in the shared background-owner state the runner lowers its process priority before launching child lanes. Use `python3 tools/check.py --serial` when debugging order, process load, or interleaved output. Use `python3 tools/check.py --list-lanes` to see lane names and repeat `--lane <name>` for focused runs. Use `--no-tool-guard` only when you intentionally want overlapping high-load local tools.
...
Three footguns: (1) never `pkill -f` broad YOLOmux patterns; kill only the listener for the selected port; (2) run `boot.sh` from the checkout you intend to serve so a dev port does not accidentally serve another worktree; (3) a backend `.py` change only takes effect after the Python process restarts, because `static_build.py` does not touch backend code. If a restarted server appears ready and then disappears, check whether readiness was served by the old listener; `boot.sh` must fail instead of launching over a still-bound old PID. The in-app self-update path likewise treats a failed, timed-out, or unlaunchable `tools/static_build.py` run as blocked and must not restart into stale generated assets; periodic update-check exceptions are logged rather than hidden.
...
Transcript metadata comes from tmux pane discovery plus local process-tree inspection. YOLOmux looks for Claude or Codex processes across the session's tmux sub-windows, finds their transcript/session metadata, and exposes both per-session summaries and per-sub-window rows in the pane header, transcript tab, popovers, Tabber, and API responses. `project.git` is the primary session summary. The backend-owned `agent_windows` rows are the source of truth for Claude/Codex tmux sub-window UI: state, pid, active sub-window, touched repo roots, and git metadata. `window_metadata[]` remains raw per-sub-window cwd metadata for transcript payload compatibility, but popover, Tabber, Info Bar, and tmux sub-window bar agent UI must not derive their agent path/git/active state from it.
...
Global durable state must use one shared storage/reader path per state family, not parallel per-server JSON files. Current durable globals are Preferences in `~/.config/yolomux/settings.yaml`, YO!agent jobs in `~/.config/yolomux/state.json`, tmux/AI status in `STATE_DIR/tmux-AI-status.json`, watch-root intent in `STATE_DIR/watch-index.json`, and search indexes in `STATE_DIR/search_index/`. `tmux-AI-status.json` owns two shared sections: `attention_acks` for cross-server red/yellow tmux AI acknowledgement state, and `stats_history` for YO!stats owner `yolomux.py` CPU, system CPU/memory, GPU device/process data, Agent status, Agent tokens/min history, transcript token sampler state, and token-consumer interest. Only the elected `stats-sampler` owner writes global stats samples; followers read the durable value when it is fresh, keep serving stale durable history if the owner is between heartbeats, and become the writer only after background-owner election promotes them. Browser latency, API/SSE counts, bandwidth/sec, disconnected time, selection/hover zoom state, and other per-client graph details are not global; they stay scoped to the browser `client_id` and are merged with the shared server fields only in the response payload.

YO!stats retains 24 hours with graduated compaction: 0–30 minutes uses one-second buckets, 30 minutes–2 hours uses ten-second buckets, 2–4 hours uses one-minute buckets, 4–8 hours uses two-minute buckets, 8–12 hours uses five-minute buckets, and 12–24 hours uses ten-minute buckets. A fully populated history family therefore has at most 2,700 time slots. `STATE_DIR/tmux-AI-status.json` stores the elected owner's shared series, while the schema-versioned `STATE_DIR/stats-client-history-vN.json` journal stores per-browser and per-server records; changing the allowed bucket durations requires a new journal version so old and new processes cannot overwrite incompatible snapshots during a rolling restart. Migration combines every available older journal from newest to oldest: current-version rows win in overlapping time ranges, older versions fill only uncovered ranges, legacy bucket durations compact into the current tiers, and duration-scaled sanity checks discard counters corrupted by the old row-duplication bug. The resulting `migrated_versions` marker prevents a restart from importing the same rows twice. Disk and heap use scale with the number of populated client/process records inside those bounded time slots, not only with elapsed time. Every compatible YOLOmux server that serves stats expands a changed shared snapshot into its own in-memory bucket maps, but file identity is checked before parsing and concurrent merges are serialized per process so repeated stats requests do not rebuild an unchanged journal. A browser retains the ranges it has fetched until the same 24-hour tier compactor removes them. Use file sizes plus bucket/client/server counts for storage audits, and measure the expanded maps rather than attributing the process's full RSS to YO!stats.
...
Search-index files live under `STATE_DIR/search_index/`. Each indexed root gets digest-named files: `<digest>.sqlite3` for entries, `<digest>.manifest.json` for cheap status/count metadata, `<digest>.lock` for the per-root build lock, `<digest>.tomb` for removals, plus SQLite WAL/SHM sidecars when SQLite needs them. Search-index followers read the small manifest for status/count metadata instead of eagerly opening the SQLite entries database; owners use the SQLite database plus WAL instead of rewriting one large JSON entries file. The browser keeps `/api/background/status` fresh so the top-right ownership chip can show `IDX|STATS|SESS: leader` when this connected process owns search-index, stats-sampler, and session-files work, or `IDX|STATS|SESS: follower` when it is reading/following another owner. `background_owner_changed` pushes update that status after election changes, and the status is refreshed on EventSource reconnect and manual refresh.

If an indexing owner dies or stops heartbeating and its process lock is gone, a follower heartbeat observes that the old generation is no longer live, acquires `owner.lock`, writes a new `owner.json`, adopts warm session-files, Tabber activity, and search-index disk snapshots into memory, and publishes `background_owner_changed` so connected clients update without waiting for a poll. If the owner heartbeat/control socket is stale but the flock is still held by a live process, a follower must not steal ownership because that would split writers; it may run one bounded local fallback for the immediate response, count and log that fallback, and then return to follower mode. Ownership is still guarded by the global owner flock, so fallback never means split ownership.

YO auto-approve is not owned by `background-owner`. It uses per-target flock files under `STATE_DIR/locks/auto-approve-<target>-<digest>.lock`, where the target is usually the agent pane target. That means the indexing owner can be one YOLOmux process while YO-on for a specific tmux pane is held by another process; the auto-approve API probes and takes over those per-target locks separately from search indexing.

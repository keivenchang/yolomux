...
Quick Open has two request modes plus an empty local view. An empty Cmd-P query performs no filesystem request and lists the newest opened file paths from the bounded browser-memory history, newest first, with each row's last-opened date/time; with no history it shows the normal empty state. An absolute or `~` path is a non-recursive direct lookup of the current containing directory; the parent listing is served through the existing bounded direct-list cache and the browser filters names locally, so changing only the filename filter does not rescan the directory. A non-absolute query uses the persistent indexed database roots. It keeps the previous rendered rows through the keystroke and replaces them on the next render, while ranking open tabs first, then the newest 100 browser-memory file-history paths, then the active AI terminal working directory, then remaining indexed matches. Indexed admission must reject unrelated absolute-path fragments before a bounded result page is filled. When coverage is incomplete or stale, it adds compact truthful status without replacing matches with `No matches`; it retries or refreshes asynchronously from producer events. A result outside published coverage is unknown, not a confirmed absence.
...
- A blocked crawler plus a cache-hit query proves the query returns within the read-path budget and is not queued behind `batchd` or indexing work.

...
- **Presence:** YO!agent Recent Agents shows active-client presence chips from `window_active_clients` / `window_active_clients_list` / parsed client rows.
...
- **Scrollback-aware snapshots:** `GET /api/tmux` calls `TmuxWebtermApp.tmux_snapshot`, which uses `history_size` / `history_bytes` to cap the `capture-pane -S` depth to the available history and returns an `unchanged` response when the target pane history signature has not grown since the last capture. Approval detection still captures live visible content and does not use this skip path.
...
## PTY / subprocess lifecycle invariants (live attach path)

Every live YOLOmux terminal attach opens a PTY and spawns `attach-session` through `bridge_tmux`. These are requirements, each learned from a concrete bug; the failure mode is listed so the requirement is not "tidied away" later.

- **Open the PTY and register inside the `try`, and close every fd on a pre-loop raise.** Any setup that can raise (`set_pty_size`, `configure_session_tmux_options`, `tmux_client_name_for_fd`, `Popen`) must run inside the `try` whose `finally`/`except` closes `master_fd`/`slave_fd` and kills the process. Setup that ran *before* the `try` leaks the openpty fds whenever it raises. Guard `attach_tmux()` with `if slave_fd is None: raise OSError(...)` so a closed PTY cannot be reattached.
- **`configure_session_tmux_options` must go through the timed `tmux()` helper, never bare `subprocess.run`.** Its bounded `set-option` calls run before the attach; an untimed call lets a hung tmux freeze the WebSocket handler. `tmux_utils.run_cmd` carries the 5s timeout and returns rc 124 on `TimeoutExpired` (`tmux_utils.py`).
- **`terminate_process_group` must `wait()` after the SIGKILL branch.** Without the `process.wait(timeout=2.0)` after `killpg(SIGKILL)` the child lingers as a zombie until CPython lazily reaps it on the next `Popen` (`yolomux_lib/infra/common.py::terminate_process_group`).
...
- **Auto-approve stops itself after `AUTO_APPROVE_MISSING_CAPTURE_LIMIT` (3) consecutive missing captures.** When the watched tmux session is killed *outside* YOLOmux, `tmux_capture_pane` returns `None`; after 3 in a row the worker emits `worker_stopped` and sets `stop_event` instead of leaking the thread/flock/registry entry forever (`yolomux_lib/approval/auto_approve_worker.py`). A successful capture resets the counter.
...
- **Repo detection should prefer real edit signals for session summaries, but per-window UI uses the window cwd.** `yolomux_lib/workspace/metadata.py::candidate_session_cwds` includes directories the session's agents actually edited, sourced from the transcript edit-tool scans in `yolomux_lib/workspace/session_files.py`, so session summaries find the real project repo even when the pane/agent cwd is `$HOME` or another non-repo. Per-window Info Bar and popover metadata must still read the selected/matching tmux sub-window cwd first, so `1:bash` does not inherit `0:codex` touched repos. The Claude and Codex scanners retain their incremental/memoized cache owners so an unchanged transcript is not reread on every metadata refresh.

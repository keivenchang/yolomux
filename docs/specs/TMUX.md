...
- **Repo detection signal:** `pane_current_path` (live cwd) is one input to a session's git-repo detection, but NOT the only one. Because an agent is frequently launched from `$HOME` or another non-repo cwd and never `cd`s, the live cwd alone yields no checkout; detection also uses agent launch cwds and the directories the agents have actually edited (from the transcript edit-tool scan), and `$HOME` is never treated as a project repo. Full contract in [`GUI.md` -> Session Project And Repo Detection](GUI.md#session-project-and-repo-detection).
...
- **Repo detection should prefer real edit signals over the live cwd.** `candidate_session_cwds` includes `session_touched_dirs(info)` (`session_files.py:358`) — directories the session's agents have actually EDITED files in, from the transcript edit-tool scan — so detection finds the real project repo even when the pane/agent cwd is `$HOME` or another non-repo. Because that scan now runs on the hot metadata-refresh path, `scan_claude_transcript` is memoized by `(path, mtime, size)` like the codex scanner (`session_files.py:85`); an unchanged transcript reads from disk once.

...
- **Window-size policy is `largest`, set at attach (`configure_session_tmux_options`, the one shared option helper both attach paths use):** YOLOmux spawns one `attach-session` client per WebSocket, so two browser surfaces on the same session attach as two differently-sized clients. Under tmux's default `latest`, the most-recently-active (often smaller) client keeps resizing the shared window — and tmux bumps `client_activity` on output too, so an active session makes the height oscillate between the two clients. Each shrink below a larger client's height leaves that client's xterm rendering tmux's now-shorter grid, and the periodic status-line redraw smears the green status bar across the orphaned rows. `window-size largest` (with `aggressive-resize on` to scope sizing to clients actually viewing the window) pins the window to the largest viewing client, so the larger view never smears or minimizes; smaller duplicate clients clip instead. Both are no-ops when only one client views the session (window == client size regardless of policy), so single-client behavior is unchanged.

Verify policy live with: `tmux show-options -g window-size` / `tmux show-options -t <session> window-size` (YOLOmux sessions are pinned to `largest`), `tmux show-window-options -g aggressive-resize` (now `on`), `tmux list-clients -F '#{client_session} #{client_width}x#{client_height} #{client_activity}'` (look for two non-control clients on one session at different sizes — the smear precondition), and `tmux list-windows -a -F '#{window_index} #{window_active_clients} #{window_activity}'`.

...
- Historical server-rendered editor markup: the removed `#fileEditorHighlight` `<pre><code>` plus `#fileEditorTextarea` nodes and their textarea-only toolbar plumbing. No current source symbol or line reference should be expected for this retired path.

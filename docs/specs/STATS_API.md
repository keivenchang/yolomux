...
| `client_id` | Required bounded opaque browser identity issued or bound by the server for request tracing and write attribution. It does not select read data; every client receives the same shared snapshot. |
...
Every frame this endpoint writes also carries an SSE `id:` line holding the server's monotonic emit timestamp in integer milliseconds, which `EventSource` exposes to the browser as `event.lastEventId`. It is diagnostic only. It sits outside the JSON payload, so no frame body key set changes and the browser's exact-field validators never see it; an in-body field would instead make every browser on a not-yet-reloaded bundle reject the frame. The browser retains it beside the frame's arrival time, which is what lets a stalled stream separate a frame the server never sent from one that arrived late. A route that does not opt in writes no `id:` line at all. The value is process-relative and non-decreasing within one connection; it is not an absolute clock and must not be compared across servers or across connections.
...
- Incremental work coalesces duplicate dirty identities, updates only affected cells, and encodes outside the writer/listener path. Unchanged coverage/no-data spans materialize zero ring cells; a changed span selects only its affected cells and view carriers, so a one-second source update is not proportional to every retained coverage gap. One-minute cutoff sweeps keep original rows within the 48-hour retention policy between configured daily maintenance occurrences: a no-change prune schedules no build, and retention deletions dirty only the cutoff cells per resolution. Writer connections use a 1000-page automatic checkpoint and an 8 MiB retained WAL allocation limit; the one quiet-gated VACUUM owner truncates its rewrite-sized WAL before and after its completion marker because logical checkpoint completion alone does not shrink the file.
- Wire encoding uses one shared 120 s grace: PUBLIC encoding stops entirely when no client asked within the grace (`status.build.encodes_skipped_idle` counts skips; the generation still publishes and previous entries are retained as stale bodies, so a returning client is served instantly and the next build re-encodes fresh); within an active grace, only DEMANDED `(range, resolution)` views encode at their live cadence while undemanded views refresh together once per 60 s. A range/resolution switch renders instantly from a <=60 s-stale retained entry and catches up on the next one-second build. Service startup counts as demand.
...
## Serving ring, rebuild lifecycle, and health

statsd owns cold serving. It persists the four resolution layers as a bounded aggregate ring in the same SQLite store, one row per resolution and bucket, with a fixed capacity per resolution. A restarted daemon reads that ring and serves from it instead of decoding retained history, so cold serving cost follows ring capacity rather than the size of the store. The ring carries an explicit invalidation ledger: a bucket whose facts changed while nothing was running is recorded as owed, and the first build after restart rebuilds exactly those cells rather than treating an unrebuilt bucket as quiet zero.

A rebuild is bounded four ways at once: by rows read, by decoded bytes held, by process memory, and by lifetime. It folds in bounded chunks and discards each decoded chunk after aggregation, so peak memory follows chunk size rather than the number of rows the rebuild covers. A build that exceeds any bound stops and leaves its remaining cells owed in the ledger; the next build picks them up. Partial progress is durable, and no build holds the whole decoded history at once.

Batched persistence is implemented and ships disabled. The buffered append path serves facts through an overlay that does not advance `source_generation`, so the served cache would carry generation 0 while showing real data, which is the state the freshness floor exists to refuse. Advancing the generation for uncommitted facts would break "no cursor, watermark or generation leads durability". That collision is recorded at `service.py:59-77` and owned by `queues/backlog/DOIT.p1.e4.statsd-batched-persistence-generation-key.md`. The arm is fail-closed: it requires a second variable that no production launch path sets, and a refused request is ignored rather than raised so a stale environment cannot take telemetry down. Treat batching as absent until that queue closes.

### Resource budgets

**A daemon's memory budget is `RssAnon + VmSwap`, never `VmRSS`.** RSS falls when the kernel swaps a process out, so a daemon can breach its budget and read healthy. Measured on the live statsd: `VmRSS` sits **61.36% below `VmHWM`** while `RssAnon + VmSwap` sits **1.08%** below it. A budget checked against RSS calls that daemon 618 MiB and healthy while it holds 1,583 MiB. A figure quoted in RSS or USS is not comparable to one quoted in the sum and must be re-derived rather than converted.

Neither health path reports PSS or USS. `/proc/<pid>/smaps_rollup` is the only source for them and it takes the target's `mmap_read_lock` for the duration, so a health check that read it would stall the daemon it is checking. The projection reports no PSS or USS field at all rather than a cheap number wearing an expensive name.
...
`/livez` asks whether the process can make progress, and nothing else. It is computed entirely from a `/proc` sample the caller reads, so it never enters statsd and never takes the GIL. That is deliberate: the materializer worker holds the GIL for the full build burst, so an endpoint that reached into the process could be blocked by the wedge it exists to detect. A process fails `/livez` when it is unreadable, when its state is not `R`, `S` or `D` — `Z` is a corpse and `T` is stopped, and neither makes progress however long you wait — or when it has outstanding work and shows no CPU or context-switch movement for longer than its stall window. That window is a multiple of the daemon's own measured build time rather than a literal, because a fixed value that is generous on today's store becomes tight on a larger one. An idle daemon is flat by definition and is never called dead.

`/readyz` asks whether the process can serve a correct snapshot right now, and it fails closed. It refuses while `cache_generation` is not yet positive, while any cell is still staged, on a ring-writer failure, on a failed materializer, while migration is not ready, when a build has failed since the last successful publication, and when startup slots are still owed. It reports **every** failing condition rather than the first, because one cause per poll costs an operator one restart cycle per cause.

`/readyz` is deliberately not a rename of `cache_ready_event`. On 6 of 6 cold starts that event fires while the served window's ring is still staged, and a snapshot requested at that instant is legitimately refused. Readiness therefore reads staged cell counts rather than the boolean queue-pending flag, which reads the same whether one cell is staged or the whole ring.
...
- Prove a cache hit performs zero SQLite scans, folds, or encoding; block a full materializer build and prove the CPU family still commits on its one-second collection cadence; exercise concurrent readers/writes/restart, bounded private-overlay eviction, and cold/warm CPU/RSS and payload measurements on the target Mac without brittle machine-specific timing assertions.

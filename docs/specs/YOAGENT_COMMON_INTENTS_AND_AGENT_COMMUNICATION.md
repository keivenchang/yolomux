...
- `periodically monitor 1 2 3 4 and if they are all done, then send a /dyn-tps-report 1 2 3 4 EOD to session 1`: create one deterministic `wait_roster_then_send` job before any model call. Watch only the roster from the condition clause, preserve the slash command exactly, require every named Claude/Codex target to be idle or done with an empty composer for the stable quiet window, and then revalidate before one server-verified send to destination session `1`. Approval, question, error, disconnect, non-agent, draft, or missing state does not count as done; a missing watched session fails visibly, and a destination that becomes busy returns the job to waiting. Do not capture a result unless the user asks for it. Flexible equivalent roster wording may invoke exactly one matching built-in model-intent skill through native JSON Schema output; unknown sessions, malformed fields, no-plan output, and ambiguous handler selection are rejected, and every accepted model-derived job requires confirmation before it can fire.
- `every 30 seconds have session 1 ask for its status 3 times`: select the `loop-send` model intent only when one known recipient is unambiguous and persist a bounded `loop_send` job without confirmation. The model may propose only a known session, prompt, five-second-to-one-hour interval, and one-to-one-hundred run count. The server owns scheduling and cancellation, verifies the target accepts an AI prompt before every send, schedules the next run only after a successful verified send, and stops on its count, timeout, cancellation, missing target, or failed send. It must never turn this into an unbounded client timer or a raw shell/tmux action.
- `have session 1 research why the test flakes`, `ask agent 1 to review the diff`, and `have session 1 validate focused tests`: route flexible research, review, and verification requests through distinct built-in model intents but one shared `wait_then_send` handler. Each planner is isolated and has no tools; it only returns a known session and clean task prompt. The recipient Claude/Codex session uses its own configured permissions for research, review, or testing. The resulting job requires confirmation and uses normal target readiness validation and verified transport.

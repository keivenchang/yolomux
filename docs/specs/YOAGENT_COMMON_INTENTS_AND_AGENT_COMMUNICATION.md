...
- `ask session 7000 for the current date`: send the target prompt, immediately report that the request was sent, keep the composer available for another request, then append the target response when captured.

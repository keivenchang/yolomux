...
- DOIT files are the active work queues; `docs/DONE.md` is the archive. As of 2026-06-14, DOIT.72's YO!share DOM replay rewrite is archived/removed and the durable replay architecture lives in `docs/specs/SHARE_MIRRORING.md`. Earlier active DOIT.63-67 and DOIT.70/71 were archived/removed, DOIT.70's YO!share architecture review moved into `docs/specs/GUI.md`, and DOIT.71's concrete Finder-session bug is archived in `docs/DONE.md`. The only carried-forward item from the older era is DOIT.11's permission-hook wiring into `~/.claude/settings.json`, which remains user-gated and tracked in Big-Bang #1.

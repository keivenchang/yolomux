...
- [ ] [XL] YO!SHARE READ-ONLY DOM REPLAY REWRITE — replace the current semantic read-only mirror with a host-rendered DOM replay path. The current model runs two app renderers and keeps them equal through hand-maintained popup selectors, UI-state fields, scroll targets, and digest components; every missed selector/field becomes a screenshot drift. Target architecture: the host is the only renderer for read-only shares; it streams a sanitized `#appRoot` DOM snapshot plus coalesced MutationObserver deltas, while terminal panes stay on the existing host-sized byte-stream/xterm path. Phase it as: (P1) spike generalized `#appRoot` serializer and `dom-snapshot`/`dom-delta` channel behind a read-only flag; (P2) keep terminal canvases as placeholders in DOM and mount viewer xterms from host bytes/dims; (P3) route read-only share view through a replay-only bundle or iframe and delete dead semantic apply/popup/snapshot/read-only-blocker code; (P4) harden redaction, high-churn exclusions, delta coalescing, and snapshot resend on backpressure; (P5) leave write clients on the current semantic path temporarily, then convert them to input-forwarding thin replayers; (P6) update docs/tests around the inverted model. Regardless of rewrite timing, collapse current share mirror code into one `shareMirror` owner, one mirrored-surface registry read by both snapshots and digests, and a `static_build.py` lint for unregistered popovers/stateful controls. Digest follow-up: add a content digest over serialized DOM minus terminal canvases/timers, move internal `mirror drift: <component>` jargon to debug UI/console, and make drift trigger the matching repair (`dom-snapshot` resend or terminal reset/refresh), not a generic state reapply.

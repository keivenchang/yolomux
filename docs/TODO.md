...
- [~] [M] PARALLELIZE + TIER the tests — mostly done: (a) `pytest -n auto` + pytest-xdist DONE (in `requirements-dev.txt`, canonical in DEVELOPMENT.md); (b) fast lane DONE via the `socket` marker (`-m "not socket"`, and `tools/check.py --fast`); (c) Selenium module-scoped driver fixture DONE (DOIT.42). REMAINING: run the slow lane (Selenium + socket tests) less often / in CI rather than per-edit.
